...
const regex = /(<div class="grid grid-cols-1 md:grid-cols-2 gap-x-4) gap-y-4 (text-sm text-gray-800 dark:text-gray-200">[\s\S]*?)<\/div>\s*<\/div>\s*<form id="profEdit_\$\{i\}"/m;

const match = code.match(regex);
if (match) {
    const originalBlock = match[0];
    
    // We will generate the new block that perfectly matches the styling of main.js
   ...
   <form id="profEdit_\$\{i\}"`;
    ...
    code = code.replace(originalBlock, newGridContent);
    fs.writeFileSync('frontend/js/profile.js', code);
    console.log("Patched profile.js layout to match main.js");
} else {
