...
function openManualGrouping(url) {
  ...
  if (typeof toggleClearBtn === "function")
    toggleClearBtn("groupingSearchInput");
...
function loadGroupingData() {
  ...
  apiCall("fetchManualPairingData", { sheetUrl: currentGroupingSheetUrl }).then(
    (res) => {
...
window.deleteGroup = function (g, event) {
  ...
  if (
    !confirm(
      `Are you sure you want to delete Group ${g}? All trainees will be unassigned and ICs removed.`,
    )
  )
    return false;
  ...
  (groupingData.trainees || []).forEach((t) => {
    if (String(t.group).trim() === String(g)) {
      t.group = "";
      changed = true;
      const updateIndex = pendingGroupingUpdates.findIndex(
        (u) => u.name === t.name && u.role === "TRAINEE",
      );
      if (updateIndex > -1) {
        pendingGroupingUpdates[updateIndex].group = "";
      } else {
        pendingGroupingUpdates.push({
          role: "TRAINEE",
          name: t.name,
          group: "",
        });
      ...
      if (t.volPaired) {
        t.volPaired
          ...
          .map((v) => v.trim())
          .filter((v) => v)
          .forEach((v) => volsInGroup.add(v.toLowerCase()));
      }
    }
  });
  ...
  (groupingData.volunteers || []).forEach((v) => {
    if (v.groupIC && volsInGroup.has(v.name.toLowerCase())) {
      v.groupIC = false;
      changed = true;
      let updateIndex = pendingGroupingUpdates.findIndex(
        (u) => u.name === v.name && u.role === "VOLUNTEER",
      );
      if (updateIndex > -1) {
  ...
  if (typeof window.extractedActiveGroups !== "undefined") {
    window.extractedActiveGroups = window.extractedActiveGroups.filter(
      (x) => String(x) !== String(g),
    );
  }
...
function renderGroupingList() {
  ...
  let activeTrainees = (groupingData.trainees || []).filter(
    (t) => t.attending === "y" && !t.isGoneHome,
  );
  ...
  activeGroups = Array.from(groupSet).sort((a, b) =>
  ...
  if (currentGroupingSearch) {
    activeTrainees = activeTrainees.filter((item) => {
      return (
        item.name.toLowerCase().includes(currentGroupingSearch) ||
        (item.volPaired &&
          item.volPaired.toLowerCase().includes(currentGroupingSearch))
      );
    });
  }
  ...
  cols.forEach((g) => {
    ...
    const colTrainees = activeTrainees.filter((t) =>
      isUnassigned
        ? String(t.group || "").trim() === ""
        : String(t.group || "").trim() === g,
    );
...
`;
    ...
    const headerBg = isUnassigned
      ? "bg-red-100 dark:bg-red-900/40 text-red-800 dark:text-red-300"
      : "bg-orange-100 dark:bg-orange-900/40 text-orange-800 dark:text-orange-300";
    const borderCol = isUnassigned
      ? "border-red-200 dark:border-red-800"
      : "border-orange-200 dark:border-orange-800";
    const innerBorderCol = isUnassigned
...
function openQuickGroupModal(traineeName) {
  ...
  if (title)
    title.innerHTML = `Group for <span class="text-orange-500">${traineeName}</span>`;
...
function getConnectedTrainees(startName) {
  ...
  while (queue.length > 0) {
    ...
    groupingData.trainees.forEach((otherT) => {
      ...
      const otherVols = otherT.volPaired
        .split(/[,|\n]+/)
        .map((v) => v.trim().toLowerCase())
        .filter((v) => v);
...
function handleGroupSelection(targetGroupRaw) {
  ...
  if (changed) {
    ...
    if (connectedTrainees.length > 1 && targetGroup !== "") {
      showFlashMessage(
        "groupingGlobalStatus",
        `Auto-Grouped ${connectedTrainees.length} trainees due to shared volunteers.`,
        "success",
      );
    }
...
function setGroupingSyncButtonState(state) {
  ...
  btn.className =
    "text-[10px] md:text-xs px-1.5 py-1 rounded font-bold transition flex items-center justify-center border shadow-md focus:outline-none shrink-0";
  spinner.className =
    "fa-solid fa-circle-notch fa-spin btn-spinner ml-1 hidden";
  ...
  if (state === "loading" || state === "saving") {
    ...
    setTimeout(() => {
      if (pendingGroupingUpdates.length === 0) {
        btn.className =
          "text-[10px] md:text-xs px-1.5 py-1 rounded font-bold transition flex items-center border-2 border-gray-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-gray-700 dark:text-gray-300 shadow-md focus:outline-none shrink-0";
        textSpan.textContent = "Saved";
      }
    }, 2000);
  } else if (state === "error") {
...
async function executeGroupingSync() {
  ...
  try {
    const res = await apiCall("syncManualGroupingUpdates", {
    ...
    updatesToSync.forEach((u) => {
      const idx = pendingGroupingUpdates.findIndex(
        (p) => p.name === u.name && p.role === u.role,
      );
...
function startGroupingPolling() {
  ...
  groupingPollInterval = setInterval(async () => {
    ...
    if (
      ...
      document.activeElement.id === "groupingSearchInput"
    )
      return;
    ...
    try {
      const res = await apiCall("fetchManualPairingData", {
        sheetUrl: currentGroupingSheetUrl,
      });
      if (
        res.success &&
        !isGroupingSyncing &&
        pendingGroupingUpdates.length === 0
      ) {
...
function openAssignICModal(sheetUrl) {
  if (!sheetUrl || sheetUrl.includes("Select"))
...
window.toggleAssignICDropdown = function (id, e) {
  ...
  document
    .querySelectorAll(".assign-ic-dropdown-list")
...
document.addEventListener("click", function (e) {
  if (
    !e.target.closest(".assign-ic-dropdown-list") &&
    !e.target.closest('button[onclick^="toggleAssignICDropdown"]')
  ) {
    document
      .querySelectorAll(".assign-ic-dropdown-list")
      .forEach((list) => list.classList.add("hidden"));
...
function generateCustomDropdownHtml(
  type,
  targetId,
  label,
  options,
...
<div class="font-bold text-sm text-gray-500">-- Clear Selection --</div>
  ...
  if (options.length === 0) {
    ...
    options.forEach((v) => {
      const remark =
        v.extra && v.extra.remark
          ? `<div class="text-[10px] text-gray-500 dark:text-gray-400 mt-0.5 whitespace-normal leading-tight break-words"><i class="fa-solid fa-note-sticky text-yellow-500 mr-1"></i>${v.extra.remark}</div>`
          : "";
...
function renderAssignICModal() {
  ...
  let activeTrainees = (groupingData.trainees || []).filter(
    (t) =>
      t.attending === "y" && !t.isGoneHome && String(t.group).trim() !== "",
  );
  let allGroups = new Set();
  activeTrainees.forEach((t) => allGroups.add(String(t.group).trim()));
  let sortedGroups = Array.from(allGroups).sort((a, b) =>
    a.localeCompare(b, undefined, { numeric: true }),
  );
  ...
  if (sortedGroups.length > 0) {
    ...
    sortedGroups.forEach((g) => {
      ...
      html += generateCustomDropdownHtml(
        "group",
        g,
        `Group ${g} IC`,
        options,
    ...
    meetingLocs.forEach((loc) => {
      ...
      (groupingData.volunteers || []).forEach((v) => {
        if (
          v.extra &&
          v.extra.v_meet &&
          v.extra.v_meet.toLowerCase() === loc.toLowerCase()
        ) {
    ...
    dismissalLocs.forEach((loc) => {
      ...
      (groupingData.volunteers || []).forEach((v) => {
        if (
          v.extra &&
          v.extra.v_dismiss &&
          v.extra.v_dismiss.toLowerCase() === loc.toLowerCase()
        ) {
...
function handleAssignICChange(type, target, newVolName) {
  ...
  if (type === "group") {
    let tList = (groupingData.trainees || []).filter(
      (t) =>
        t.attending === "y" &&
        !t.isGoneHome &&
        String(t.group).trim() === String(target),
    );
    tList.forEach((t) => {
      if (t.volPaired) {
        t.volPaired
          ...
          .map((v) => v.trim())
          .filter((v) => v)
          .forEach((v) => groupVolKeys.add(v.toLowerCase()));
      }
    });
  } else if (type === "meet") {
    (groupingData.volunteers || []).forEach((v) => {
      if (
        v.extra &&
        v.extra.v_meet &&
        v.extra.v_meet.toLowerCase() === target.toLowerCase()
      ) {
    ...
    (groupingData.volunteers || []).forEach((v) => {
      if (
        v.extra &&
        v.extra.v_dismiss &&
        v.extra.v_dismiss.toLowerCase() === target.toLowerCase()
      ) {
        groupVolKeys.add(v.name.toLowerCase());
      }
    });
  ...
  (groupingData.volunteers || []).forEach((v) => {
    ...
    if (groupVolKeys.has(vKey)) {
      ...
      let changed = false;
      if (type === "group" && v.groupIC !== !!shouldBeIC) {
        v.groupIC = !!shouldBeIC;
        changed = true;
      } else if (type === "meet" && v.meetIC !== !!shouldBeIC) {
        v.meetIC = !!shouldBeIC;
        changed = true;
      } else if (type === "dismiss" && v.dismissIC !== !!shouldBeIC) {
        v.dismissIC = !!shouldBeIC;
        changed = true;
      }
      ...
      if (changed) {
        ...
        if (type === "meet")
          pendingGroupingUpdates[updateIndex].meetIC = !!shouldBeIC;
        if (type === "dismiss")
          pendingGroupingUpdates[updateIndex].dismissIC = !!shouldBeIC;
      }
    }
  });
...
function openTableExportModal() {
  ...
  if (btn) {
    ...
    btn.className =
      "px-4 md:px-5 py-2 md:py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-bold flex items-center gap-2 shadow-md transition-colors";
    btn.disabled = false;
  }
...
function buildExportTable() {
  ...
  sortedGroups.forEach((g, index) => {
    ...
    let tList = groupingData.trainees.filter(
      (t) =>
        t.attending === "y" && !t.isGoneHome && String(t.group).trim() === g,
    );
        ...
        vols.forEach((v) => {
          ...
          if (!volMap.has(vKey)) {
            ...
            volMap.set(vKey, {
              ...
              volRemark: vObj
                ? vObj.extra?.remarks || vObj.extra?.remark || ""
                : "",
              trainees: [],
              remarks: [],
            });
          ...
          if (remarks)
            vData.remarks.push(`<strong>[Trn] ${t.name}:</strong> ${remarks}`);
        });
      } else {
...
<td style="padding: 6px 4px; border: 1px solid #ccc; vertical-align: middle; text-align: left; line-height: 1.3; font-size: 11px; color: #333;"></td>
    ...
    rows.forEach((r) => {
      ...
      if (r.isGroupIC)
        volDisplay += `<br><strong style="color: #0369a1; font-size: 0.9em; display:inline-block; margin-top:2px;">(Grp ${g} IC)</strong>`;
      if (r.isMeetIC) {
      ...
      if (r.isDismissIC) {
        const locDisplay = r.dismissLoc
          ? `Dismissal - ${r.dismissLoc}`
          : "Dismissal";
      ...
      let allRemarks = [];
      if (r.volRemark)
        allRemarks.push(`<strong>[Vol] ${r.name}:</strong> ${r.volRemark}`);
...
<td contenteditable="true" style="padding: 6px 4px; border: 1px solid #ccc; outline: none; transition: background 0.2s; vertical-align: middle; text-align: left; line-height: 1.3; font-size: 11px; color: #333;" onfocus="this.style.backgroundColor='#fff'" onblur="this.style.backgroundColor='transparent'">${rDisplay}</td>
    ...
    unpairedTrainees.forEach((ut) => {
      let rDisplay = ut.remarks
        ? `<strong>[Trn] ${ut.name}:</strong> ${ut.remarks}`
        : "";
      allTrs.push(`<tr style="background-color: ${bgColor};">
...
<td contenteditable="true" style="padding: 6px 4px; border: 1px solid #ccc; outline: none; transition: background 0.2s; vertical-align: middle; text-align: left; line-height: 1.3; font-size: 11px; color: #333;" onfocus="this.style.backgroundColor='#fff'" onblur="this.style.backgroundColor='transparent'">${rDisplay}</td>
  ...
  let unassignedTrainees = groupingData.trainees.filter(
    (t) =>
      t.attending === "y" && !t.isGoneHome && String(t.group).trim() === "",
  );
  ...
  let unpairedVols = groupingData.volunteers.filter(
    (v) => !displayedVols.has(v.name.toLowerCase()),
  );
  ...
  if (allTrs.length === 0) {
    allTrs.push(
      `<tr><td colspan="4" style="padding: 10px; text-align: center; font-style: italic;">No data.</td></tr>`,
    );
  }
...
async function shareExportTable() {
  ...
  setTimeout(async () => {
    try {
      ...
      btn.className =
        "px-4 md:px-5 py-2 md:py-2.5 bg-green-600 hover:bg-green-700 text-white rounded-lg text-sm font-bold flex items-center gap-2 shadow-md transition-colors";
...
async function executeNativeShare() {
  ...
  const filesArray = generatedImageBlobs.map((blob, index) => {
    return new File([blob], `outing-groups-page-${index + 1}.png`, {
      type: "image/png",
    });
  });
  ...
  try {
    if (navigator.canShare && navigator.canShare({ files: filesArray })) {
      ...
      if (navigator.clipboard && window.ClipboardItem) {
        const clipboardItems = generatedImageBlobs.map(
        ...
        alert(
          "Sharing not supported on this device. Long press the images to save them.",
        );
      }
    }
  } catch (e) {
