...
if (!window.dndInitialized) {
  ...
  function startDrag(e, clientX, clientY, isTouch) {
    ...
    if (!draggable && !roomDraggable && !groupDraggable && !busDraggable)
      return;
    ...
    if (draggable) {
      ...
      if (
        !pairingContainer ||
        pairingContainer.classList.contains("hidden-force")
      )
        return;
      ...
      if (!roomsContainer || roomsContainer.classList.contains("hidden-force"))
        return;
      ...
      if (
        !groupsContainer ||
        groupsContainer.classList.contains("hidden-force")
      )
        return;
      ...
      if (!busesContainer || busesContainer.classList.contains("hidden-force"))
        return;
      dndState.type = "busing";
      dndState.el = busDraggable;
    }
    dndState.nameNode =
      dndState.el.querySelector(".main-name-pill") || dndState.el;
  ...
  function moveDrag(e, clientX, clientY, isTouch) {
    ...
    if (dndState.isDragging && dndState.clone) {
      ...
      if (dndState.type === "pairing") {
        ...
        const activeGroup = elAtPoint
          ? elAtPoint.closest(".dnd-group-dropzone")
          : null;
        ...
        const activeBus = elAtPoint
          ? elAtPoint.closest(".dnd-bus-dropzone")
          : null;
        ...
        const activeRoom = elAtPoint
          ? elAtPoint.closest(".dnd-room-dropzone")
          : null;
  ...
  function endDrag(e, clientX, clientY) {
    ...
    if (dndState.isDragging && dndState.clone) {
      ...
      document
        .querySelectorAll(
          ".dnd-dropzone, .dnd-room-dropzone, .dnd-group-dropzone, .dnd-bus-dropzone",
        )
        .forEach((dz) =>
          dz.classList.remove(
            "border-primary",
      ...
      if (dndState.type === "pairing") {
        ...
        if (
          dropZone &&
          dndState.el &&
          dropZone.dataset.role !== dndState.el.dataset.role
        ) {
          const sourceNric = dndState.el.dataset.nric;
          const sourceRole = dndState.el.dataset.role;
          const targetNric = dropZone.dataset.nric;
          if (sourceNric && targetNric)
            handleDndDrop(sourceNric, sourceRole, targetNric);
        }
      } else if (dndState.type === "rooming") {
        const roomDropZone = elAtPoint
          ? elAtPoint.closest(".dnd-room-dropzone")
          : null;
        if (roomDropZone && dndState.el) {
          const sourceNric = dndState.el.dataset.nric;
          const targetRoomId = roomDropZone.dataset.roomId;
          if (sourceNric && targetRoomId)
            handleRoomDrop(sourceNric, targetRoomId);
        }
      } else if (dndState.type === "grouping") {
        const groupDropZone = elAtPoint
          ? elAtPoint.closest(".dnd-group-dropzone")
          : null;
        if (groupDropZone && dndState.el) {
          ...
          const targetGroup =
            groupDropZone.id === "groupUnassignedPool"
              ? ""
              : groupDropZone.dataset.group;
          if (sourceNric && typeof targetGroup !== "undefined")
            handleGroupDrop(sourceNric, targetGroup);
        }
      } else if (dndState.type === "busing") {
        const busDropZone = elAtPoint
          ? elAtPoint.closest(".dnd-bus-dropzone")
          : null;
        if (busDropZone && dndState.el) {
          const sourceNric = dndState.el.dataset.nric;
          const targetBus =
            busDropZone.id === "busUnassignedPool"
              ? ""
              : busDropZone.dataset.bus;
          if (sourceNric && typeof targetBus !== "undefined")
            handleBusDrop(sourceNric, targetBus);
        }
      }
    }
    dndState.el = null;
...
function handleDndDrop(sourceNric, sourceRole, targetNric) {
  ...
  let tPerson = globalLogistics.participants.find(
    (p) => p.nric === traineeNric,
  );
  let vGroup =
    vPerson && vPerson.logisticsGroup
      ? String(vPerson.logisticsGroup).trim().toLowerCase()
      : "";
  let tGroup =
    tPerson && tPerson.logisticsGroup
      ? String(tPerson.logisticsGroup).trim().toLowerCase()
      : "";
  ...
  if (
    ...
    showToast(
      "Cannot pair: Trainee and Volunteer must be in the same group, or one must be unassigned.",
      true,
    );
    return;
  }

  let existing = globalLogistics.pairings.find(
    (p) => p.traineeNric === traineeNric && p.volNric === volNric,
  );
...
function setSyncButtonState(state) {
  ...
  btns.forEach((btn) => {
    ...
    btn.className =
      "btn-sync-pairings text-xs md:text-xs px-2 py-1 rounded-md font-bold transition flex items-center justify-center border shadow-md focus:outline-none shrink-0";
...
async function executePairingSync() {
  ...
  try {
    const res = await apiCall("syncPairingUpdates", {
      updates: updates,
    ...
    if (res.pairings) {
      res.pairings.forEach((sPair) => {
        ...
        if (!pendingPairingsMap.has(key)) {
          let lPair = globalLogistics.pairings.find(
            (p) =>
              p.traineeNric === sPair.traineeNric &&
              p.volNric === sPair.volNric,
          );
          if (lPair) {
...
function startPairingPolling() {
  ...
  pairingPollInterval = setInterval(async () => {
    ...
    if (
      !logTab ||
      logTab.classList.contains("hidden-force") ||
      isPairingSyncing ||
      pendingPairingsMap.size > 0 ||
      (dndState.type === "pairing" && (dndState.el || dndState.isDragging))
    )
      return;
    ...
    try {
      ...
      if (res.pairings) {
        ...
        if (hasChanges && !dndState.el && !dndState.isDragging) {
          ...
          if (
            sheet &&
            !sheet.classList.contains("hidden-force") &&
            currentPairingTarget
          ) {
...
function setRoomSyncButtonState(state) {
  ...
  const spinner = btn.querySelector(".btn-spinner");
  btn.className =
    "text-xs md:text-xs px-2 py-1 rounded-md font-bold transition flex items-center justify-center border shadow-md focus:outline-none shrink-0";
  spinner.className = "btn-spinner ml-1 !w-3 !h-3 hidden-force";
  if (state === "loading") {
    btn.classList.add(
      "bg-gray-100",
...
function addRoom() {
  const prefix = prompt(
    "Enter Room Base Name (e.g. 'Room', 'Tent', 'Block A'):",
    "Room",
  );
  ...
  let existingCount = globalLogistics.rooms.filter(
    (r) => !r.isDeleted && r.name.startsWith(prefix.trim()),
  ).length;
...
function deleteRoom(id) {
  if (
    !confirm(
      "Are you sure you want to delete this room? Users inside will be unassigned.",
    )
  )
    return;
  const room = globalLogistics.rooms.find((r) => r.id === id);
...
function deleteAllRooms() {
  if (
    !confirm(
      "⚠️ DANGER: This will delete ALL rooms and unassign everyone. Are you sure you want to proceed?",
    )
  )
    return;
  let changed = false;
...
function resetRoomAssignments() {
  if (!confirm("Are you sure you want to unassign everyone from all rooms?"))
    return;
  let changed = false;
  globalLogistics.rooms.forEach((r) => {
...
function autoAssignRooms() {
  if (
    !confirm(
      "This will automatically assign unassigned participants into EXISTING rooms based on their connections. Continue?",
    )
  )
    return;
  ...
  function getRoomState(roomId) {
    ...
    r.occupants.forEach((n) => {
      ...
      if (p) {
        ...
        if (window.isFamily(p.nric, globalLogistics.participants))
          hasFamily = true;
  ...
  let placedCount = 0;
  const unassigned = globalLogistics.participants.filter(
    (p) => !roomAssignments[p.nric],
  );
  if (unassigned.length === 0) {
    showToast("Everyone is already assigned.");
    return;
  }
  ...
  remainingUnassigned.forEach((p) => {
    if (!visited.has(p.nric)) {
      ...
      while (q.length > 0) {
        const curr = q.shift();
        if (!visited.has(curr)) {
          const pObj = globalLogistics.participants.find(
            (x) => x.nric === curr,
          );
  ...
  pairClusters.forEach((cluster) => {
    let cGender = "empty";
    const gSet = new Set(
      cluster.map((p) => (p.gender ? p.gender.toLowerCase() : "")),
    );
...
async function executeRoomSync() {
  ...
  try {
    const res = await apiCall("syncRoomUpdates", {
      updates: updates,
      takenBy: currentUser.name,
    });
    if (res.rooms) {
...
function startRoomPolling() {
  ...
  roomPollInterval = setInterval(async () => {
    ...
    if (
      ...
      logTab.classList.contains("hidden-force") ||
      !roomSec ||
      roomSec.classList.contains("hidden-force") ||
      isRoomSyncing ||
      pendingRoomUpdates.size > 0 ||
      (dndState.type === "rooming" && (dndState.el || dndState.isDragging))
    )
      return;
...
<div id="log-pairings" class="flex-1 flex flex-col min-h-0 w-full relative">
    ...
    <div class="bg-white dark:bg-gray-900 border-b-2 border-gray-200 dark:border-gray-700 p-2 md:p-3 shrink-0 flex flex-col gap-1 shadow-md sticky top-0 z-30">
        <div class="flex justify-between items-center px-1">
            <div class="flex items-center gap-2">
                <h3 class="text-sm md:text-base font-black text-gray-900 dark:text-white tracking-tight">Pairings</h3>
                <button onclick="toggleAltSwap()" class="bg-gray-100 dark:bg-gray-800 p-1 rounded-md hover:bg-gray-200 dark:hover:bg-gray-700 transition focus:outline-none border-2 border-gray-200 dark:border-gray-700 shadow-md" title="Swap Columns">
            ...
            <button onclick="manualSyncPairings(this)" class="btn-sync-pairings text-xs md:text-xs px-2 py-1 rounded-md font-bold transition flex items-center justify-center border shadow-md bg-green-50 text-green-700 border-green-200 dark:bg-green-900/30 dark:text-green-300 dark:border-green-800 focus:outline-none shrink-0">
        ...
        <div class="flex justify-between items-center px-1 gap-2 mt-1 mb-1">
            <div class="relative w-full">
                <input type="text" id="pairingSearchInput" oninput="renderPairings()" placeholder="Search pairings..." class="w-full p-1.5 pl-7 pr-8 border-2 border-gray-300 dark:border-gray-700 rounded text-xs font-bold bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary shadow-md transition">
    ...
    <div class="flex flex-row flex-1 min-h-0 w-full overflow-hidden relative bg-gray-50 dark:bg-gray-950 border-x border-b-2 border-gray-200 dark:border-gray-700 rounded-b-xl md:rounded-none">
        <div id="dnd-source-col" class="flex-1 min-w-0 flex flex-col h-full overflow-hidden transition-colors border-r-2 border-gray-200 dark:border-gray-700">
...
<div id="log-rooms" class="hidden-force flex-1 flex flex-col min-h-0 w-full relative">
    <div class="bg-white dark:bg-gray-900 border-b-2 border-gray-200 dark:border-gray-700 p-2 md:p-3 shrink-0 flex flex-col gap-2 shadow-md sticky top-0 z-30">
        <div class="flex justify-between items-center px-1">
            <div class="flex flex-wrap items-center gap-1 md:gap-1.5">
                ...
                <button onclick="deleteAllRooms()" class="bg-red-50 text-red-600 border-2 border-red-200 dark:bg-red-900/30 dark:text-red-400 dark:border-red-800 text-[11px] md:text-xs font-bold px-1.5 py-1 md:px-2 md:py-1.5 rounded shadow-md hover:bg-red-100 transition focus:outline-none flex items-center gap-0.5 md:gap-1" title="Delete All Rooms">
                    <svg class="w-3 h-3 md:w-3.5 md:h-3.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
        ...
        <div class="flex justify-between items-center px-1 gap-2 mt-1">
            <div class="relative w-full max-w-sm">
                <input type="text" id="roomSearchInput" oninput="renderRooms()" placeholder="Search participants/rooms..." class="w-full p-1.5 pl-7 pr-8 border-2 border-gray-300 dark:border-gray-700 rounded text-xs font-bold bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary shadow-md transition">
                <svg class="w-3.5 h-3.5 absolute left-2 top-2 text-gray-400 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                <button onclick="clearSearch('roomSearchInput', 'renderRooms')" class="absolute right-1.5 top-1.5 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 focus:outline-none"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg></button>
            </div>
            <select id="roomFilterSelect" onchange="renderRooms()" class="text-xs font-bold bg-white dark:bg-gray-800 border-2 border-gray-300 dark:border-gray-600 rounded px-1.5 py-1.5 outline-none shrink-0">
    ...
    <div class="flex flex-row flex-1 min-h-0 w-full overflow-hidden relative bg-gray-50 dark:bg-gray-950 rounded-b-xl md:rounded-none">
        <div class="flex-1 min-w-0 flex flex-col h-full overflow-hidden border-r-2 border-gray-200 dark:border-gray-700 bg-gray-100/50 dark:bg-gray-900/50">
            <h4 class="font-black text-xs py-1.5 shrink-0 text-center uppercase tracking-widest bg-gray-200/50 dark:bg-gray-800/50 text-gray-600 dark:text-gray-400 border-b-2 border-gray-200 dark:border-gray-700">Unassigned (<span id="unassignedCount">0</span>)</h4>
        ...
        <div class="flex-1 min-w-0 flex flex-col h-full overflow-hidden transition-colors bg-white dark:bg-gray-950">
            <div class="flex items-center justify-between px-2 py-1.5 shrink-0 border-b-2 border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900/50">
                ...
                <button onclick="openManageRoomsSheet()" class="px-2 py-1 bg-white dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-700 rounded shadow-md text-xs font-bold text-gray-600 dark:text-gray-300 hover:text-primary hover:border-primary transition focus:outline-none"><i class="fa-solid fa-cog"></i></button>
            </div>
...
<div id="log-groups" class="hidden-force flex-1 flex flex-col min-h-0 w-full relative">
    ...
    <div class="flex flex-row flex-1 min-h-0 w-full overflow-hidden relative">
        <div class="flex-1 min-w-0 flex flex-col h-full overflow-hidden transition-colors border-r-2 border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900">
        ...
        <div class="flex-1 min-w-0 flex flex-col h-full overflow-hidden transition-colors bg-white dark:bg-gray-950">
            <div class="flex items-center justify-between px-2 py-1.5 shrink-0 border-b-2 border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900/50">
                ...
                <button onclick="openManageGroupsSheet()" class="px-2 py-1 bg-white dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-700 rounded shadow-md text-xs font-bold text-gray-600 dark:text-gray-300 hover:text-primary hover:border-primary transition focus:outline-none"><i class="fa-solid fa-cog"></i></button>
            </div>
    ...
    <div class="flex flex-row flex-1 min-h-0 w-full overflow-hidden relative">
        ...
        <div class="flex-1 min-w-0 flex flex-col h-full overflow-hidden transition-colors bg-white dark:bg-gray-950">
            <div class="flex items-center justify-between px-2 py-1.5 shrink-0 border-b-2 border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900/50">
                ...
                <button onclick="openManageBusesSheet()" class="px-2 py-1 bg-white dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-700 rounded shadow-md text-xs font-bold text-gray-600 dark:text-gray-300 hover:text-primary hover:border-primary transition focus:outline-none"><i class="fa-solid fa-cog"></i></button>
            </div>
...
function renderGroups() {
  ...
  globalLogistics.participants.forEach((p) => {
    ...
    if (query) {
      ...
      match =
        ...
        p.nric.toLowerCase().includes(query) ||
        pGroup.toLowerCase().includes(query);
    } else {
      match = true;
    }
  ...
  if (window.sortParticipantsSpecial)
    window.sortParticipantsSpecial(unassigned, globalLogistics.participants);
  let unHtml = "";
  unassigned.forEach((item) => {
    unHtml += generateGroupCardHtml(item);
  });
  const el_groupUnassignedPool = document.getElementById("groupUnassignedPool");
  if (el_groupUnassignedPool)
    el_groupUnassignedPool.innerHTML =
      unHtml ||
      '<p class="text-xs text-gray-500 font-bold p-2 text-center mt-2">All assigned / No matches.</p>';
  ...
  activeGroupsList.forEach((gName) => {
    ...
    grpHtml += `
        <div class="dnd-group-dropzone bg-white dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-700 rounded-lg p-2 shadow-md transition-colors" data-group="${gName}">
            <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center border-b-2 border-gray-100 dark:border-gray-700 pb-1.5 mb-1.5 gap-2 w-full">
                <div class="flex items-start justify-between w-full lg:w-auto gap-2 flex-1">
                    <span class="font-black text-[12px] md:text-sm text-gray-900 dark:text-white break-words whitespace-normal leading-tight uppercase tracking-wider">Group ${gName}</span>
                ...
                <div class="flex items-center gap-1 shrink-0 w-full lg:w-auto justify-end">
                    <button onclick="openGroupICSheet('${gName.replace(/'/g, "\\\x27")}')" class="text-[11px] bg-yellow-50 text-yellow-600 border-2 border-yellow-200 dark:bg-yellow-900/30 dark:text-yellow-400 dark:border-yellow-800 font-bold px-1.5 py-0.5 rounded hover:bg-yellow-100 transition focus:outline-none flex items-center gap-1"><i class="fa-solid fa-crown"></i> IC</button><button onclick="openGroupAddSheet('${gName.replace(/'/g, "\\'")}')" class="text-[11px] bg-green-50 text-green-600 border-2 border-green-200 dark:bg-green-900/30 dark:text-green-400 dark:border-green-800 font-bold px-1.5 py-0.5 rounded hover:bg-green-100 transition focus:outline-none">+ Add</button>
  ...
  const el_groupListContainer = document.getElementById("groupListContainer");
  if (el_groupListContainer) el_groupListContainer.innerHTML = grpHtml;
}
...
function renderBuses() {
  ...
  globalLogistics.participants.forEach((p) => {
    ...
    if (query) {
      ...
      match =
        ...
        fullName.includes(query) ||
        p.nric.toLowerCase().includes(query) ||
        pBus.toLowerCase().includes(query);
    } else {
      match = true;
    }
  ...
  if (el_busUnassignedPool)
    el_busUnassignedPool.innerHTML =
      unHtml ||
      '<p class="text-xs text-gray-500 font-bold p-2 text-center mt-2">All assigned / No matches.</p>';
  ...
  if (el_busListContainer) el_busListContainer.innerHTML = busHtml;
}
...
window.handleGroupDrop = async function (nric, groupName) {
  ...
  if (p.isGroupIC && p.logisticsGroup !== groupName) {
    const confirmMsg =
      "This participant is currently the Group IC of Group " +
      (p.logisticsGroup || "Unknown") +
    ...
    p.isGroupIC = false;
    try {
      await apiCall("syncAssignments", {
        updates: [{ nric: nric, value: false }],
        column: "isGroupIC",
      });
    } catch (e) {
...
function getConnectedParticipants(startNric) {
  ...
  const activePairings = (globalLogistics.pairings || []).filter(
...
function autoGroup() {
  if (activeGroupsList.length === 0)
    return showToast("Please add at least one group first.");
  let unassigned = globalLogistics.participants.filter(
    (p) => !p.logisticsGroup,
  );
...
function autoBus() {
  if (activeBusesList.length === 0)
    return showToast("Please add at least one bus first.");
    ...
    if (res.participants) {
      ...
      res.participants.forEach((sPart) => {
        if (!pendingGroupUpdates.has(sPart.nric)) {
          let lPart = globalLogistics.participants.find(
            (p) => p.nric === sPart.nric,
          );
...
async function loadLogisticsData() {
  ...
  try {
    ...
    console.error(e);
    showToast("Logistics error: " + (e.message || e), true);
    setSyncButtonState("error");
    setRoomSyncButtonState("error");
  } finally {
    if (overlay) overlay.classList.add("hidden-force");
...
<div class="${targetColorClass} text-xs md:text-sm pl-2 pr-6 py-1 rounded shadow-md border-2 border-gray-300 dark:border-gray-600 font-bold opacity-90 leading-tight break-words whitespace-normal text-left w-full">
${targetName}
</div>
...
function generateCardHtml(
  ...
  const isFam =
    item.role === "TRAINEE" &&
    traineesWithCaregivers.has(String(item.name).trim().toLowerCase());
  const famBadge = isFam
    ? `<span class="bg-purple-100 dark:bg-purple-900 text-purple-800 dark:text-purple-200 text-[10px] uppercase font-black tracking-wider px-1 py-0.5 rounded shrink-0 shadow-md border-2 border-purple-200 dark:border-purple-700 pointer-events-none whitespace-nowrap">FAM</span>`
    : "";

  const myPairings =
    item.role === "TRAINEE"
      ? activePairings.filter((p) => p.traineeNric === item.nric)
      : activePairings.filter((p) => p.volNric === item.nric);

  let pairedPills = "";
  myPairings.forEach((pair) => {
    const pairedPerson =
      item.role === "TRAINEE"
        ? vols.find((v) => v.nric === pair.volNric)
        : trainees.find((t) => t.nric === pair.traineeNric);
    if (pairedPerson) {
      const pColor = getProjectColor(pairedPerson.group);
      pairedPills += generatePillHtml(
        pairedPerson.displayName || pairedPerson.name,
        pColor,
        pair.traineeNric,
        pair.volNric,
      );
  ...
  return `
<div class="dnd-draggable dnd-dropzone bg-white dark:bg-gray-800 p-1.5 md:p-2 rounded-md border-2 border-gray-200 dark:border-gray-700 shadow-[0_1px_2px_rgba(0,0,0,0.05)] cursor-grab active:cursor-grabbing hover:border-primary transition select-none flex flex-col min-h-[60px] gap-1" data-nric="${item.nric}" data-role="${item.role}">
    <div class="flex justify-between items-start w-full gap-1">
        <div class="main-name-pill font-extrabold text-sm md:text-[12px] px-1.5 py-0.5 rounded shadow-md border ${dynColor} max-w-full inline-flex flex-wrap items-center gap-1 self-start min-w-0 leading-[1.1]">
            <span class="break-words whitespace-normal min-w-0 text-left">${displayName}</span>
            ${famBadge}
        </div>
        <button onclick="openPairingSheet('${item.nric}', '${item.role}')" class="text-[11px] md:text-xs bg-green-50 dark:bg-gray-700 text-green-600 dark:text-green-400 font-bold px-1.5 py-1 rounded border-2 border-green-200 dark:border-gray-600 hover:bg-green-100 transition whitespace-nowrap focus:outline-none shrink-0 pointer-events-auto shadow-md">${btnLabel}</button>
    </div>
...
function renderPairings() {
  ...
  const vols = globalLogistics.participants.filter(
    (p) => p.role === "VOLUNTEER",
  );
  const activePairings = (globalLogistics.pairings || []).filter(
    (p) => !p.status || p.status === "ACTIVE",
  ...
  if (hidePairedVols) {
    ...
    if (isSourceVol)
      sourceArr = sourceArr.filter((p) => !pairedVolNrics.has(p.nric));
    else targetArr = targetArr.filter((p) => !pairedVolNrics.has(p.nric));
  }
  if (hidePairedTrainees) {
    const pairedTraineeNrics = new Set(
      activePairings.map((p) => p.traineeNric),
    );
    if (!isSourceVol)
      sourceArr = sourceArr.filter((p) => !pairedTraineeNrics.has(p.nric));
    else targetArr = targetArr.filter((p) => !pairedTraineeNrics.has(p.nric));
  }

  const query = document.getElementById("pairingSearchInput")
    ? document.getElementById("pairingSearchInput").value.toLowerCase().trim()
    : "";
  if (query) {
    const matchFn = (p) => {
      const dName = (p.displayName || p.name).toLowerCase();
  ...
  const volTitleClass =
    "bg-orange-100 dark:bg-orange-900/50 text-orange-800 dark:text-orange-300 border-b-2 border-orange-200 dark:border-orange-800";
  const traineeTitleClass =
    "bg-green-100 dark:bg-green-900/50 text-green-800 dark:text-green-300 border-b-2 border-green-200 dark:border-green-800";

  const sourceCol = document.getElementById("dnd-source-col");
  const targetCol = document.getElementById("dnd-target-col");
  sourceCol.className = `flex-1 min-w-0 flex flex-col h-full overflow-hidden transition-colors border-r-2 border-gray-200 dark:border-gray-700 ${sourceColClass}`;
  ...
  const volLabel = `<div class="flex items-center justify-between px-2 w-full"><span class="w-16"></span><span class="flex-1 text-center">Volunteers</span><label class="flex items-center gap-1.5 cursor-pointer w-[76px] justify-end" onclick="event.stopPropagation()"><input type="checkbox" ${hidePairedVols ? "checked" : ""} onchange="toggleHidePaired('VOLUNTEER', this)" class="w-4 h-4 text-orange-600 focus:ring-orange-500 rounded-sm cursor-pointer border-orange-300 dark:border-orange-700 bg-white dark:bg-gray-800"><span class="text-[11px] font-bold tracking-normal normal-case opacity-80 mt-[1px]">Unpaired</span></label></div>`;
  const traineeLabel = `<div class="flex items-center justify-between px-2 w-full"><span class="w-16"></span><span class="flex-1 text-center">Trainees</span><label class="flex items-center gap-1.5 cursor-pointer w-[76px] justify-end" onclick="event.stopPropagation()"><input type="checkbox" ${hidePairedTrainees ? "checked" : ""} onchange="toggleHidePaired('TRAINEE', this)" class="w-4 h-4 text-green-600 focus:ring-green-500 rounded-sm cursor-pointer border-green-300 dark:border-green-700 bg-white dark:bg-gray-800"><span class="text-[11px] font-bold tracking-normal normal-case opacity-80 mt-[1px]">Unpaired</span></label></div>`;
  ...
  const el_dnd_source_pool = document.getElementById("dnd-source-pool");
  if (el_dnd_source_pool)
    el_dnd_source_pool.innerHTML =
      sourceHtml ||
      '<p class="text-xs text-gray-500 font-bold p-2 text-center mt-2">No items.</p>';

  let targetHtml = "";
  targetArr.forEach((item) => {
    targetHtml += generateCardHtml(
      item,
      traineesWithCaregivers,
      activePairings,
      vols,
  ...
  const el_dnd_target_list = document.getElementById("dnd-target-list");
  if (el_dnd_target_list)
    el_dnd_target_list.innerHTML =
      targetHtml ||
      '<p class="text-xs text-gray-500 font-bold p-2 text-center mt-2">No items.</p>';
}
...
function renderRooms() {
  ...
  const rFilter = document.getElementById("roomFilterSelect")
    ? document.getElementById("roomFilterSelect").value
    : "ALL";
  const query = document.getElementById("roomSearchInput")
    ? document.getElementById("roomSearchInput").value.toLowerCase().trim()
    : "";
  ...
  activeRooms.forEach((r) =>
    r.occupants.forEach((n) => allNricsInRooms.add(n)),
  );

  const unassignedArr = globalLogistics.participants.filter(
    (p) =>
      !allNricsInRooms.has(p.nric) && (rFilter === "ALL" || p.role === rFilter),
  );
  ...
  document.getElementById("unassignedCount").innerText =
  ...
  filteredUnassigned.forEach((item) => {
    ...
    const sleepingTooltip = item.sleeping
      ? `Request: ${item.sleeping.replace(/"/g, "&quot;").replace(/'/g, "&#39;")}`
      : "";
    const sleepingIndicator = item.sleeping
      ? `<button onclick="openSleepingModal('${item.nric}')" class="ml-1 text-indigo-500 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 pointer-events-auto transition-transform hover:scale-110 focus:outline-none shrink-0" title="${sleepingTooltip}"><svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M12 3a9 9 0 109 9c0-.46-.04-.92-.1-1.36a5.389 5.389 0 01-4.4 2.26 5.403 5.403 0 01-3.14-9.8c-.44-.06-.9-.1-1.36-.1z"/></svg></button>`
      : "";

    unHtml += `
    <div class="dnd-room-draggable bg-white dark:bg-gray-800 p-1 md:p-1.5 rounded-md border-2 border-gray-200 dark:border-gray-700 shadow-md cursor-grab active:cursor-grabbing hover:border-primary transition select-none flex flex-col gap-1" data-nric="${item.nric}" data-role="${item.role}">
        <div class="main-name-pill font-extrabold text-xs md:text-sm px-1.5 py-1 rounded shadow-md border ${dynColor} w-full flex items-start justify-between gap-1">
            <span class="break-words whitespace-normal text-left flex-1">${item.isGroupIC ? "<i class=\x27fa-solid fa-crown text-amber-500 mr-1\x27></i>" : ""}${dName}</span>
            ${sleepingIndicator}
        </div>
  ...
  const el_roomUnassignedPool = document.getElementById("roomUnassignedPool");
  if (el_roomUnassignedPool)
    el_roomUnassignedPool.innerHTML =
      unHtml ||
      '<p class="text-xs text-gray-500 font-bold p-2 text-center mt-2">All assigned / No matches.</p>';

  let roomsToRender = activeRooms;
  if (window.showUnfilledRoomsOnly) {
    roomsToRender = roomsToRender.filter(
  ...
  roomsToRender.forEach((room) => {
    ...
    room.occupants.forEach((nric) => {
      ...
      if (p) {
        ...
        const matchClass = isMatch
          ? "ring-2 ring-primary ring-offset-1 dark:ring-offset-gray-800 scale-105 z-10"
          : "";

        const pSleepingTooltip = p.sleeping
          ? `Request: ${p.sleeping.replace(/"/g, "&quot;").replace(/'/g, "&#39;")}`
          : "";
        const sleepingIndicator = p.sleeping
          ? `<button onclick="openSleepingModal('${p.nric}')" class="ml-1 text-indigo-500 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 pointer-events-auto transition-transform hover:scale-110 focus:outline-none shrink-0" title="${pSleepingTooltip}"><svg class="w-3 h-3 md:w-3.5 md:h-3.5 inline-block" fill="currentColor" viewBox="0 0 24 24"><path d="M12 3a9 9 0 109 9c0-.46-.04-.92-.1-1.36a5.389 5.389 0 01-4.4 2.26 5.403 5.403 0 01-3.14-9.8c-.44-.06-.9-.1-1.36-.1z"/></svg></button>`
          : "";
    ...
    const occDisplay = isFull
      ? `<span class="text-green-600 dark:text-green-400 font-black tracking-widest">${occCount}/${room.capacity} (FULL)</span>`
      : `<span class="text-gray-500 dark:text-gray-400 font-bold tracking-widest">${occCount}/${room.capacity}</span>`;

    roomHtml += `
    <div class="dnd-room-dropzone flex flex-col bg-white dark:bg-gray-800 rounded-xl border-2 border-gray-200 dark:border-gray-700 shadow-md transition-colors" data-room-id="${room.id}">
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center bg-gray-50/80 dark:bg-gray-900/50 p-2 border-b-2 border-gray-100 dark:border-gray-700 rounded-t-xl shrink-0 gap-2 w-full">
            <div class="flex items-start justify-between w-full lg:w-auto gap-2 flex-1">
                ...
                <span class="text-[11px] bg-gray-200/50 dark:bg-gray-700/50 px-1.5 py-0.5 rounded border-2 border-gray-300 dark:border-gray-600 shrink-0 mt-0.5">${occDisplay}</span>
            </div>
            <div class="flex items-center gap-1 shrink-0 w-full lg:w-auto justify-end">
                <button onclick="openRoomAddSheet('${room.id}')" class="text-[11px] bg-green-50 text-green-600 border-2 border-green-200 dark:bg-green-900/30 dark:text-green-400 dark:border-green-800 font-bold px-1.5 py-0.5 rounded hover:bg-green-100 transition focus:outline-none" ${isFull ? 'disabled style="opacity:0.5;"' : ""}>+ Add</button>
  ...
  const el_roomListContainer = document.getElementById("roomListContainer");
  if (el_roomListContainer)
    el_roomListContainer.innerHTML =
      roomHtml ||
      '<div class="flex justify-center items-center h-20 text-xs font-bold text-gray-400">No rooms match criteria.</div>';
}
...
function openSleepingModal(nric) {
  ...
  if (modalTitle)
    modalTitle.textContent = `Request: ${p.displayName || p.name}`;
...
function openPairingSheet(sourceNric, sourceRole = "TRAINEE") {
  ...
  const sourcePerson = globalLogistics.participants.find(
    (p) => p.nric === sourceNric,
  );
  let titleHtml =
    sourceRole === "TRAINEE" ? "Select Volunteer" : "Select Trainee";

  if (sourcePerson) {
    const dynColor = getProjectColor(sourcePerson.group);
    const dName = sourcePerson.displayName || sourcePerson.name;
    titleHtml = `Pair with <span class="ml-1 font-bold text-sm md:text-xs px-1.5 py-0.5 rounded shadow-md border ${dynColor}">${dName}</span>`;
  }

  const el_sheetTitle = document.getElementById("sheetTitle");
  if (el_sheetTitle) el_sheetTitle.innerHTML = titleHtml;
  const searchInput = document.getElementById("sheetSearchInput");
  if (searchInput) searchInput.value = "";
  document
    .getElementById("selectionBottomSheet")
    .classList.remove("hidden-force");

  const targetRole = sourceRole === "TRAINEE" ? "VOLUNTEER" : "TRAINEE";
  const targets = globalLogistics.participants.filter(
    (p) => p.role === targetRole,
  );
  const activePairings = (globalLogistics.pairings || []).filter(
    (p) => !p.status || p.status === "ACTIVE",
  ...
  targets.forEach((t) => {
    const isPaired =
      sourceRole === "TRAINEE"
        ? activePairings.some(
            (p) => p.volNric === t.nric && p.traineeNric === sourceNric,
          )
        : activePairings.some(
            (p) => p.traineeNric === t.nric && p.volNric === sourceNric,
          );
    ...
    const roleColor =
      t.role === "VOLUNTEER"
        ...
        : "text-green-700 bg-green-100 dark:bg-green-900/50 border-green-200 dark:border-green-800";
    const dName = t.displayName || t.name;
    const fullName = (t.name || "").toLowerCase();

    html += `<div onclick="confirmPairing('${t.nric}')" class="sheet-list-item flex flex-col bg-white dark:bg-gray-800 p-2.5 rounded-lg border-2 border-gray-200 dark:border-gray-700 shadow-[0_1px_2px_rgba(0,0,0,0.05)] cursor-pointer hover:border-primary transition mb-1.5" data-name="${dName.toLowerCase()}" data-fullname="${fullName}">
    <div class="flex justify-between items-start w-full gap-2">
        <span class="font-extrabold text-sm md:text-xs px-1.5 py-0.5 rounded shadow-md border ${tDynColor} break-words whitespace-normal min-w-0 flex-1 text-left leading-[1.1]">${dName}</span>
  ...
  if (el_sheetListContainer)
    el_sheetListContainer.innerHTML =
      html ||
      `<p class="text-xs font-bold text-gray-400 p-2 text-center mt-2">No available options.</p>`;
}
...
function openRoomAddSheet(roomId) {
  ...
  const el_sheetTitle = document.getElementById("sheetTitle");
  if (el_sheetTitle)
    el_sheetTitle.innerHTML = `Add to <span class="ml-1 font-black text-primary">${room.name}</span>`;
  const searchInput = document.getElementById("sheetSearchInput");
  if (searchInput) searchInput.value = "";
  document
    .getElementById("selectionBottomSheet")
  ...
  unassignedArr.forEach((t) => {
    ...
    const roleColor =
      t.role === "TRAINEE"
        ? "text-green-700 bg-green-100 dark:bg-green-900/50 border-green-200 dark:border-green-800"
        : t.role === "CAREGIVER"
          ? "text-purple-700 bg-purple-100 dark:bg-purple-900/50 border-purple-200 dark:border-purple-800"
          : "text-orange-700 bg-orange-100 dark:bg-orange-900/50 border-orange-200 dark:border-orange-800";
    const dName = t.displayName || t.name;
    const fullName = (t.name || "").toLowerCase();

    html += `<div onclick="confirmRoomAdd('${t.nric}')" class="sheet-list-item flex flex-col bg-white dark:bg-gray-800 p-2.5 rounded-lg border-2 border-gray-200 dark:border-gray-700 shadow-[0_1px_2px_rgba(0,0,0,0.05)] cursor-pointer hover:border-primary transition mb-1.5" data-name="${dName.toLowerCase()}" data-fullname="${fullName}">
    <div class="flex justify-between items-start w-full gap-2">
        <span class="font-extrabold text-sm md:text-xs px-1.5 py-0.5 rounded shadow-md border ${tDynColor} break-words whitespace-normal min-w-0 flex-1 text-left leading-[1.1]">${dName}</span>
  ...
  if (el_sheetListContainer)
    el_sheetListContainer.innerHTML =
      html ||
      `<p class="text-xs font-bold text-gray-400 p-2 text-center mt-2">Everyone is assigned.</p>`;
}
...
function filterBottomSheet() {
  ...
  items.forEach((item) => {
    if (
      ...
      (item.dataset.fullname && item.dataset.fullname.includes(query))
    ) {
...
function confirmPairing(targetNric) {
  ...
  const traineeNric =
    currentPairingSourceRole === "TRAINEE" ? currentPairingTarget : targetNric;
  const volNric =
    currentPairingSourceRole === "TRAINEE" ? targetNric : currentPairingTarget;
...
function renderGroupBusOptions() {
  const query = document
    .getElementById("sheetSearchInput")
    .value.toLowerCase()
    .trim();
  let list = [];
  if (activeAssignType === "group") list = activeGroupsList;
    ...
    const activeRooms = (globalLogistics.rooms || []).filter(
      (r) => !r.isDeleted,
    );
    activeRooms.sort((a, b) =>
      a.name.localeCompare(b.name, undefined, {
    ...
    list = activeRooms
      .map((r) => ({ id: r.id, name: r.name, ts: r.ts }))
  ...
  if (activeAssignNric && (!query || "unassign".includes(query))) {
    html += `
        <div onclick="selectGroupBusOption('')" class="sheet-list-item cursor-pointer p-3 rounded-xl border-2 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-md flex items-center justify-between hover:bg-gray-50 dark:hover:bg-gray-750 transition" data-name="unassign">
  ...
  list.forEach((item) => {
    ...
    if (activeAssignType === "group") {
      const currentICs = globalLogistics.participants.filter(
        (p) => p.logisticsGroup === nameStr && p.isGroupIC,
      );
      let icName =
        currentICs.length > 0
          ? currentICs
              .map((c) => c.displayName || c.name || "Unknown")
              .join(", ")
          : "None";
      if (!activeAssignNric) {
        // Manage Mode
  ...
  const el_sheetListContainer = document.getElementById("sheetListContainer");
  if (el_sheetListContainer) el_sheetListContainer.innerHTML = html;
}
  ...
  if (activeAssignType === "group") {
    if (!activeGroupsList.includes(val)) {
      ...
      localStorage.setItem(
        "activeGroupsList",
        JSON.stringify(activeGroupsList),
      );
    }
  } else {
...
function openManageGroupsSheet() {
  ...
  const el_sheetTitle = document.getElementById("sheetTitle");
  if (el_sheetTitle)
    el_sheetTitle.innerHTML = `Manage <span class="text-primary">Groups</span>`;
  const searchInput = document.getElementById("sheetSearchInput");
  if (searchInput) searchInput.value = "";
  renderGroupBusOptions();
  document
...
function openManageBusesSheet() {
  ...
  const el_sheetTitle = document.getElementById("sheetTitle");
  if (el_sheetTitle)
    el_sheetTitle.innerHTML = `Manage <span class="text-primary">Buses</span>`;
  const searchInput = document.getElementById("sheetSearchInput");
  if (searchInput) searchInput.value = "";
  renderGroupBusOptions();
  document
...
function openManageRoomsSheet() {
  ...
  const el_sheetTitle = document.getElementById("sheetTitle");
  if (el_sheetTitle)
    el_sheetTitle.innerHTML = `Manage <span class="text-primary">Rooms</span>`;
  const searchInput = document.getElementById("sheetSearchInput");
  if (searchInput) searchInput.value = "";
  renderGroupBusOptions();
  document
...
function setGroupSyncButtonState(state) {
  ...
  const spinner = btn.querySelector(".btn-spinner");
  btn.className =
    "btn-sync-groups text-xs md:text-xs px-2 py-1 rounded-md font-bold transition flex items-center justify-center border shadow-md focus:outline-none shrink-0";
  spinner.className = "btn-spinner ml-1 !w-3 !h-3 hidden-force";
  if (state === "loading") {
    btn.classList.add(
      "bg-gray-100",
...
function setBusSyncButtonState(state) {
  ...
  const spinner = btn.querySelector(".btn-spinner");
  btn.className =
    "btn-sync-buses text-xs md:text-xs px-2 py-1 rounded-md font-bold transition flex items-center justify-center border shadow-md focus:outline-none shrink-0";
  spinner.className = "btn-spinner ml-1 !w-3 !h-3 hidden-force";
  if (state === "loading") {
    btn.classList.add(
      "bg-gray-100",
...
window.showRoomInfoPopup = function () {
  ...
  overlay.className =
    "fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 transition-opacity";
  overlay.onclick = (e) => {
...
window.openGroupAddSheet = function (gName) {
  ...
  if (el_sheetListContainer)
    el_sheetListContainer.innerHTML =
      html ||
      '<div class="text-center p-4 text-gray-500 font-bold text-sm">No available participants.</div>';
};
...
window.openGroupICSheet = function (gName) {
  const el_sheetTitle = document.getElementById("sheetTitle");
  if (el_sheetTitle)
    el_sheetTitle.innerHTML = `Assign ICs for <span class="ml-1 font-black text-primary">${gName}</span>`;
  const searchInput = document.getElementById("sheetSearchInput");
  if (searchInput) searchInput.value = "";
  document
    .getElementById("selectionBottomSheet")
  ...
  const assignedArr = globalLogistics.participants.filter(
    (p) => p.logisticsGroup === gName && p.role === "VOLUNTEER",
  );
  let html = "";
  ...
  if (el_sheetListContainer)
    el_sheetListContainer.innerHTML =
      html ||
      '<p class="text-sm font-medium text-gray-400 p-2 text-center">No volunteers assigned to this group.</p>';
};
...
window.assignICToGroupFromSheet = async function (nric, gName) {
  ...
  try {
    ...
    showToast(
      newValue
        ? "Group IC assigned successfully."
        : "Group IC removed successfully.",
    );
  } catch (e) {
