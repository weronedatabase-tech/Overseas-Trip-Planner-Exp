...
function buildAttendanceUI() {
    ...
    <div class="admin-only flex flex-col h-full min-h-0 w-full relative">
        ...
        <div class="flex flex-row flex-1 min-h-0 w-full overflow-hidden relative bg-zinc-50 dark:bg-zinc-950 rounded-b-xl md:rounded-none border-x border-b border-zinc-200 dark:border-zinc-800">
            ...
            <div id="attLoadingOverlay" class="absolute inset-0 bg-white/60 dark:bg-zinc-900/60 backdrop-blur-sm z-10 hidden-force flex flex-col justify-center items-center">
...
function generateAttCard(p, isChecked) {
    const dynColor = typeof getProjectColor === 'function' ? getProjectColor(p.group) : 'bg-zinc-100';
