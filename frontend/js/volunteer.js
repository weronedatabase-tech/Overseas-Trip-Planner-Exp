...
function loadVolunteerEvents() {
  ...
  const renderSelectedEvent = (data) => {
    ...
    if (!selectedItem && data.length > 0) {
      const closestUrl = window.getClosestEventUrl
    ...
    if (selectedItem) {
      ...
      if (nameEl)
        nameEl.innerText =
          selectedItem.displayName || selectedItem.fullName || "Upcoming Event";
      if (dateTextEl)
        dateTextEl.innerText = selectedItem.formattedDate || "N/A";
    ...
    if (res && res.success && res.data) {
      ...
      if (dateTextEl)
        dateTextEl.innerText =
          res && res.message ? res.message : "Please refresh";
    }
  });
}
...
function loadVolNames(requestedType) {
  ...
  if (!url || !requestedType || url === "Select an Event" || !list || !input)
    return;
  ...
  list.innerHTML = Array(5)
    .fill(
      '<li class="px-4 py-3 border-b-2 border-gray-200 dark:border-zinc-700"><div class="animate-pulse h-4 bg-gray-200 dark:bg-zinc-700 rounded w-2/3"></div></li>',
    )
    .join("");
...
function toggleSearchList(show) {
  ...
  if (show) {
    list.classList.remove("hidden");
    if (document.getElementById("volNameSearch").value.length > 0)
      filterNames();
  } else {
...
function filterNames() {
  ...
  matches.forEach((name) => {
    ...
    li.className =
      "px-4 py-3 bg-white dark:bg-zinc-800 text-gray-900 dark:text-gray-100 border-b-2 border-gray-200 dark:border-zinc-700 hover:bg-cyan-600 hover:text-white cursor-pointer text-sm transition-colors last:border-0";
    li.innerText = name;
  ...
  if (matches.length === 0) {
    const li = document.createElement("li");
    li.className =
      "px-4 py-3 text-sm text-gray-500 dark:text-gray-400 italic bg-white dark:bg-zinc-800";
...
function getValueFuzzy(dataObj, lookupKey) {
  ...
  for (let key in dataObj) {
    ...
    if (cleanLookup.includes("caregiver") && cleanKey.includes("caregiver"))
      return dataObj[key];
  }
  return "";
}
...
function filterActiveVols() {
  ...
  const matches = (currentActiveVols || []).filter(
    (v) =>
  ...
  matches.forEach((match) => {
    const li = document.createElement("li");
    li.className =
      "px-3 py-2 bg-white dark:bg-zinc-800 text-gray-900 dark:text-gray-100 border-b-2 border-gray-200 dark:border-zinc-700 hover:bg-teal-600 hover:text-white cursor-pointer text-xs transition-colors last:border-0";
    li.innerText = match;
    li.onmousedown = (e) => {
      e.preventDefault();
      addVolPaired(match);
  ...
  if (matches.length === 0) {
    const li = document.createElement("li");
    li.className =
      "px-3 py-2 text-xs text-gray-500 dark:text-gray-400 italic bg-white dark:bg-zinc-800";
    li.innerText = `Press Enter to add "${input.value.trim()}"`;
    li.onmousedown = (e) => {
      e.preventDefault();
      addVolPaired(input.value.trim());
    };
    list.appendChild(li);
  }
}
...
function updateVolPairedUI() {
  ...
  tagsContainer.innerHTML = currentVolPairedValue
    .map((v) => {
      const jsSafeVol = v
        .replace(/\\/g, "\\\\")
        .replace(/'/g, "\\'")
...
function loadVolData(name, isManualNew) {
  ...
  fieldsDiv.innerHTML = Array(4)
    .fill(
      '<div class="mb-2"><div class="animate-pulse h-3 bg-gray-200 dark:bg-zinc-700 rounded w-1/4 mb-1.5"></div><div class="animate-pulse h-10 bg-gray-100 dark:bg-zinc-800 rounded w-full"></div></div>',
    )
    .join("");
    ...
    if (res.success) {
      ...
      let fieldsToShow =
        config && config.length > 0
          ? config
          : res.headers.map((h) => h.replace(/\[.*?\]/g, "").trim());
      if (isNew) {
        const nameFieldExists = fieldsToShow.some((f) =>
          f.toLowerCase().includes("name"),
        );
        if (!nameFieldExists) {
          const rawNameHeader =
            res.headers.find((h) => h.toLowerCase().includes("name")) ||
            "Volunteer Name";
      ...
      fieldsToShow.forEach((header) => {
        ...
        if (!isNameField && !cleanH.includes("attending")) {
          wrapperClass += " attendance-dependent";
        }
        ...
        if (cleanH.includes("attending")) {
          ...
          cleanH.includes("meetinglocation") ||
          cleanH.includes("dismissallocation")
        ) {
          const isDismissal = cleanH.includes("dismissal");
          const optionsList = isDismissal ? dismissalOpts : meetingOpts;
          const placeholder = isDismissal
            ? "Select Dismissal..."
            : "Select Meeting...";
          let optionsHtml = optionsList
            .map((opt) => {
              const isSelected =
          ...
          if (valTrimmed !== "") {
            const listHasValue = optionsList.some(
              (opt) =>
                opt.toString().trim().toLowerCase() ===
                valTrimmed.toLowerCase(),
            );
            if (!listHasValue) {
      ...
      const attSelect = document.querySelector(
        'select[name*="Attending"], select[name*="attending"]',
      );
...
function submitAttendance(e) {
  ...
  const payload = {
    sheetUrl: document.getElementById("volSheetSelector").value,
    type: selectedVolType,
    data: deltaObj,
    targetName: target,
  };
