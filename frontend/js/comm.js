...
function ensureMeetingJuncture() {
  ...
  if (!commAttData.attendance["Meeting"])
    commAttData.attendance["Meeting"] = {};
  if (!commAttData.attendance["__GONE_HOME__"])
    commAttData.attendance["__GONE_HOME__"] = {};
  ...
  commAttData.busJunctures.forEach((bj) => {
    ...
    if (
      ...
      commAttData.busAttendance[compositeName] = Object.assign(
        {},
        commAttData.busAttendance[bj.name],
      );
    }
...
function loadSheets(viewId, forceRefresh = false) {
  ...
  const renderData = (data) => {
    ...
    if (selector) {
      ...
      const closest = window.getClosestEventUrl
        ? window.getClosestEventUrl(data)
        : data[0].sheetUrl;
      if (closest) {
    ...
    if (viewId === "volunteer" && typeof resetVolForm === "function") {
      ...
      if (typeof launchLiveTracker === "function")
        setTimeout(() => launchLiveTracker(), 100);
    }
    ...
    if (res.success) {
      ...
      if (res.data.length > 0) {
        ...
        if (selector) {
          selector.disabled = false;
          selector.innerHTML =
            "<option disabled selected>No upcoming events</option>";
        }
        if (viewId === "comm" && listContainer) {
          listContainer.innerHTML =
            '<p class="text-xs text-gray-500 dark:text-gray-400 italic">No upcoming outings found.</p>';
        }
      }
    } else {
      if (selector) {
  ...
  const fetchBatchStats = () => {
    ...
    if (batch.length === 0 || currentIndex >= MAX_STATS_TO_FETCH) {
      for (let i = currentIndex; i < data.length; i++) {
        ...
        if (container) {
          container.innerHTML =
            '<span class="text-gray-400 italic text-[10px]">Stats skipped to preserve quota</span>';
...
function fetchOutingStats(url, index) {
  return apiCall("getOutingDetails", url).then((res) => {
    ...
    if (res.success) {
      outingDetailsCache[index] = {
        config: res.outingConfig,
        message: res.outingMessage,
      };

      let html =
        '<table class="w-full text-[10px] md:text-xs text-left border-collapse"><tr class="text-gray-500 dark:text-gray-400 border-b-2 border-gray-200 dark:border-zinc-700"><th class="pb-1">Proj</th><th class="text-center pb-1">Trainees</th><th class="text-center pb-1">CG</th><th class="text-center pb-1">Vols</th></tr>';
      ...
      if (sortedKeys.length === 0) {
        html +=
          '<tr><td colspan="4" class="text-center py-3 text-gray-400 dark:text-gray-500 italic">No data yet</td></tr>';
      } else {
...
function copyFromModal(btn) {
  performCopy(document.getElementById("modalReminderText").value, btn);
}
...
function handleCreate(e) {
  ...
  const formData = {
    ...
    meetingLocs: Array.from(document.getElementsByName("meetingLoc")).map(
      (i) => i.value,
    ),
    meetingTimes: Array.from(document.getElementsByName("meetingTime")).map(
      (i) => i.value,
    ),
    meetingBuses: Array.from(document.getElementsByName("meetingBus")).map(
      (i) => i.checked,
    ),
    dismissalLocs: Array.from(document.getElementsByName("dismissalLoc")).map(
      (i) => i.value,
    ),
    dismissalTimes: Array.from(document.getElementsByName("dismissalTime")).map(
      (i) => i.value,
    ),
    dismissalBuses: Array.from(document.getElementsByName("dismissalBus")).map(
      (i) => i.checked,
    ),
  };
...
function openEditOutingModal(index) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    dBuses[i].checked =
      config.dismissalBuses && config.dismissalBuses[i] === true;
  }
...
function handleEditSubmit(e) {
  ...
  const formData = {
    ...
    meetingLocs: Array.from(document.getElementsByName("editMeetingLoc")).map(
      (i) => i.value,
    ),
    meetingTimes: Array.from(document.getElementsByName("editMeetingTime")).map(
      (i) => i.value,
    ),
    meetingBuses: Array.from(document.getElementsByName("editMeetingBus")).map(
      (i) => i.checked,
    ),
    dismissalLocs: Array.from(
      document.getElementsByName("editDismissalLoc"),
    ).map((i) => i.value),
    dismissalTimes: Array.from(document.getElementsByName("dismissalTime")).map(
      (i) => i.value,
    ),
    dismissalBuses: Array.from(
      document.getElementsByName("editDismissalBus"),
    ).map((i) => i.checked),
  };

  apiCall("updateOuting", {
    sheetUrl: currentEditSheetUrl,
    form: formData,
  }).then((res) => {
    if (res.success) {
...
function loadCommAttendanceData() {
  ...
  apiCall("fetchCommAttendance", { sheetUrl: currentCommAttSheetUrl }).then(
    (res) => {
...
function renderCommAttFilters() {
  ...
  updateCommAttFilterUI(
    "dismiss",
    sortedDismissals,
    commAttState.selectedDismissals,
  );
}
...
function updateCommAttFilterUI(type, availableItems, selectedArray) {
  ...
  const dropdownId =
    type === "group"
      ? "commAttGroupDropdown"
      : type === "meet"
        ? "commAttMeetDropdown"
        : "commAttDismissDropdown";
  ...
  let btnText =
    type === "group" ? "Grp: " : type === "meet" ? "Meeting: " : "Dismissal: ";
...
function toggleCommAttFilterItem(type, item, e) {
  ...
  let targetArray =
    type === "group"
      ? commAttState.selectedGroups
      : type === "meet"
        ? commAttState.selectedMeets
        : commAttState.selectedDismissals;
  const available =
    type === "group"
      ? commAttState.availableGroups
      : type === "meet"
        ? commAttState.availableMeets
        : commAttState.availableDismissals;
...
function renderCommAttJunctures() {
  ...
  if (
    commAttState.currentJuncture &&
    commAttData.junctures.includes(commAttState.currentJuncture)
  ) {
...
function renderCommAttLists() {
  ...
  participants.forEach((p) => {
    const isGoneHome =
      commAttData.attendance["__GONE_HOME__"] &&
      commAttData.attendance["__GONE_HOME__"][p.name] === true;
  ...
  if (commAttState.selectedGroups.length > 0) {
    participants = participants.filter((p) =>
      commAttState.selectedGroups.includes(String(p.group)),
    );
  }
  if (commAttState.selectedMeets.length > 0) {
    participants = participants.filter((p) =>
      commAttState.selectedMeets.includes(String(p.meetingLoc)),
    );
  }
  if (commAttState.selectedDismissals.length > 0) {
    participants = participants.filter((p) =>
      commAttState.selectedDismissals.includes(String(p.dismissalLoc)),
    );
  }
  ...
  participants.forEach((p) => {
    const isGoneHome =
      ...
      commAttData.attendance["__GONE_HOME__"][p.name] === true;
    const isChecked =
      juncture && commAttData.attendance[juncture]
        ? commAttData.attendance[juncture][p.name] === true
        : false;
    ...
    uiBindLongPress(el, () => {
      ...
      if (p) {
        if (commAttState.searchQuery) {
          ...
          if (typeof toggleClearBtn === "function")
            toggleClearBtn("commAttSearchInput");
          renderCommAttLists();
        }
        showPersonInfo(p);
      }
...
function generateCommAttCard(p, isChecked, isGoneHome) {
  ...
  const caregiverBadge =
    p.caregivers > 0
      ? `<span class="inline-flex shrink-0 items-center justify-center min-w-[16px] md:min-w-[20px] h-4 md:h-5 px-1 bg-red-500 rounded-full text-[9px] md:text-[11px] font-black text-white shadow-md mt-px" title="${p.caregivers} Caregiver(s)">${p.caregivers > 1 ? p.caregivers + "C" : "C"}</span>`
      : "";

  let volHtml = "";
  if (p.volPaired) {
    const vols = p.volPaired
      .split(/[,|\n]+/)
      .map((v) => v.trim())
      .filter((v) => v);
    if (vols.length > 0) {
      volHtml = vols
        .map(
          (v) =>
            `<span class="text-[9px] md:text-[11px] text-teal-700 dark:text-teal-400 leading-tight font-bold bg-teal-50 dark:bg-teal-900/30 px-1.5 md:px-2 py-0.5 md:py-1 rounded border-2 border-teal-200 dark:border-teal-800/50 whitespace-normal break-words w-fit max-w-full text-left"><i class="fa-solid fa-handshake-angle mr-1"></i>${v}</span>`,
        )
        .join("");
    }
  } else if (!isGoneHome) {
  ...
  if (p.extra && p.extra.t_one_on_one) {
    ...
    if (
      oneOnOneRaw !== "" &&
      !["no", "n", "false", "0"].includes(oneOnOneRaw)
    ) {
  ...
  if (p.meetingLoc || p.dismissalLoc) {
    locHtml =
      '<div class="flex flex-col gap-1 w-full mt-1 border-t-2 border-gray-100 dark:border-zinc-700/60 pt-1.5">';
    if (p.meetingLoc) {
  ...
  const groupBadge = p.group
    ? `<span class="bg-orange-100 text-orange-800 border-orange-200 dark:bg-orange-900/40 dark:text-orange-300 dark:border-orange-800 border px-1.5 md:px-2 py-0.5 md:py-1 rounded font-black text-[9px] md:text-[11px] uppercase shadow-md whitespace-nowrap">Grp ${p.group}</span>`
    : "";

  const homeBtnClass = isGoneHome
    ? "bg-blue-500 text-white border-blue-600 shadow-inner"
    : "bg-gray-100 dark:bg-zinc-800 text-gray-500 dark:text-gray-400 border-gray-200 dark:border-zinc-700 hover:text-blue-500 dark:hover:text-blue-400 hover:border-blue-500";
  const checkBtnClass = isChecked
    ? "bg-green-500 border-green-600 text-white shadow-inner"
    : "bg-gray-50 dark:bg-black border-gray-300 dark:border-zinc-600 text-transparent";
  ...
  if (existingCard) {
    ...
    if (currentListId === targetListId) {
      ...
      existingCard.style.transition =
        "opacity 0.2s ease, height 0.3s ease, margin 0.3s ease, padding 0.3s ease";
...
function rebindCommAttCard(cardId, p) {
  ...
  if (card) {
    uiBindLongPress(card, () => {
      const pObj = (commAttData.participants || []).find(
        (x) => x.name === p.name,
      );
      if (pObj) {
        if (commAttState.searchQuery) {
...
function insertCardSorted(listEl, newCardNode, pData) {
  const children = Array.from(listEl.children).filter(
    (c) =>
      c.classList.contains("comm-att-card") ||
      c.classList.contains("bus-att-card"),
  );
  let inserted = false;
  for (let child of children) {
    ...
    if (childP) {
      ...
      const grpB = childP.group
        ? childP.group.toString().toLowerCase()
        : "zzzz";
      let groupCmp = grpA.localeCompare(grpB, undefined, { numeric: true });
      if (
        groupCmp < 0 ||
        (groupCmp === 0 && pData.name.localeCompare(childP.name) < 0)
      ) {
...
function updateCommAttCountsDOM() {
  ...
  (commAttData.participants || []).forEach((p) => {
    ...
    if (
      commAttState.selectedGroups.length > 0 &&
      !commAttState.selectedGroups.includes(String(p.group))
    )
      return;
    if (
      commAttState.selectedMeets.length > 0 &&
      !commAttState.selectedMeets.includes(String(p.meetingLoc))
    )
      return;
    if (
      commAttState.selectedDismissals.length > 0 &&
      !commAttState.selectedDismissals.includes(String(p.dismissalLoc))
    )
      return;
  ...
  if (commAttState.searchQuery) {
    ...
    setTimeout(() => {
      const cardNode = document.getElementById(
        `comm-att-card-${name.replace(/[^a-zA-Z0-9]/g, "")}`,
      );
      if (cardNode) {
        scrollCardIntoViewLocally(cardNode);
        const isGoneHome =
          commAttData.attendance["__GONE_HOME__"] &&
          commAttData.attendance["__GONE_HOME__"][name] === true;
        const pulseClass = isGoneHome
          ? "pulse-blue"
          : forceState
            ? "pulse-green"
            : "pulse-red";
...
function toggleGoneHomeStatus(name, forceState, e) {
  ...
  if (!pendingCommAttUpdates["__GONE_HOME__"])
    pendingCommAttUpdates["__GONE_HOME__"] = {};
  ...
  if (commAttState.searchQuery) {
    ...
    setTimeout(() => {
      ...
      if (cardNode) {
        ...
        const isChecked =
          juncture && commAttData.attendance[juncture]
            ? commAttData.attendance[juncture][name] === true
            : false;
        const pulseClass = forceState
          ? "pulse-blue"
          : isChecked
            ? "pulse-green"
            : "pulse-red";
...
function generateColumnText(columnType) {
  ...
  participants.forEach((p) => {
    ...
    else if (columnType === "checked" && !isGoneHome && isChecked)
      targetNames.push(p.name);
    else if (columnType === "notChecked" && !isGoneHome && !isChecked)
      targetNames.push(p.name);
  });

  const groupsStr =
    commAttState.selectedGroups.length > 0
      ? "Grp: " + commAttState.selectedGroups.join(", ")
      : "All Groups";
  const meetsStr =
    commAttState.selectedMeets.length > 0
      ? "Meeting: " + commAttState.selectedMeets.join(", ")
      : "All Meetings";
  const dismissStr =
    commAttState.selectedDismissals.length > 0
      ? "Dismissal: " + commAttState.selectedDismissals.join(", ")
      : "All Dismissals";
...
async function executeCommAttSync() {
  ...
  for (let junc in pendingCommAttUpdates) {
    ...
    for (let name in pendingCommAttUpdates[junc]) {
      payloadUpdates[junc].push({
        name: name,
        status: pendingCommAttUpdates[junc][name],
      });
    }
  }
  ...
  try {
    const res = await apiCall("syncCommAttendance", {
      sheetUrl: currentCommAttSheetUrl,
      multipleUpdates: payloadUpdates,
    });
    if (res.success) {
...
function setCommAttBtnState(state) {
  ...
  btn.className =
    "text-[10px] md:text-xs px-1.5 py-1 flex items-center justify-center rounded font-bold transition border-2 border-gray-300 dark:border-zinc-700 focus:outline-none shadow-md shrink-0";
...
function promptNewCommJuncture() {
  ...
  apiCall("addCommJuncture", {
    sheetUrl: currentCommAttSheetUrl,
    junctureName: name.trim(),
  }).then((res) => {
    overlay.classList.add("hidden");
    if (res.success) {
      commAttData = res;
...
function promptDeleteCommJuncture() {
  ...
  if (!confirm(`Are you sure you want to delete the juncture "${juncture}"?`))
    return;
  ...
  apiCall("deleteCommJuncture", {
    sheetUrl: currentCommAttSheetUrl,
    junctureName: juncture,
  }).then((res) => {
    overlay.classList.add("hidden");
    if (res.success) {
      commAttData = res;
  ...
  apiCall("fetchCommAttendance", { sheetUrl: currentCommAttSheetUrl }).then(
    (res) => {
      ...
      if (res.success) {
        ...
        if (
          !document
            .getElementById("busTrackerModal")
        ...
        if (res.success && !isCommAttSyncing && !hasPendingUpdates()) {
          ...
          if (
            ...
            if (oldBusJunctures !== newBusJunctures) {
              ...
              oldBusAttendance !== newBusAttendance ||
              oldParticipants !== newParticipants
            ) {
              renderBusLists();
            }
...
function handleCommAttSearch() {
  const query = document
    .getElementById("commAttSearchInput")
    .value.toLowerCase()
    .trim();
...
function renderBusJunctures() {
  ...
  if (juncs.length === 0) {
    select.innerHTML =
      '<option value="" disabled selected>No buses catered.</option>';
  ...
  if (
    busState.currentJuncture &&
    validValues.includes(busState.currentJuncture)
  ) {
...
function removeBusOption() {
  ...
  if (
    !confirm(
      `Are you sure you want to remove ${filterBus}? All trainees assigned to this bus will be unassigned.`,
    )
  ) {
    return;
  }
  ...
  if (commAttData.busAttendance[juncture]) {
    for (let name in commAttData.busAttendance[juncture]) {
      if (commAttData.busAttendance[juncture][name] === filterBus) {
        ...
        if (!pendingCommAttUpdates[juncKey])
          pendingCommAttUpdates[juncKey] = {};
...
function getEligibleBusTrainees() {
  ...
  if (juncture.startsWith("Meeting: ")) {
    ...
    const defaultJunc = (commAttData.busJunctures || []).find(
      (b) => b.name === juncture,
    );
    if (defaultJunc) {
  ...
  if (matchMeet) {
    return participants.filter(
      (p) => String(p.meetingLoc).toLowerCase() === rawLocName.toLowerCase(),
    );
  } else if (matchDismiss) {
    return participants.filter(
      (p) => String(p.dismissalLoc).toLowerCase() === rawLocName.toLowerCase(),
    );
  }
...
function renderBusLists() {
  ...
  if (busState.searchQuery) {
    ...
    eligibleTrainees = eligibleTrainees.filter(
      (p) =>
        p.name.toLowerCase().includes(query) ||
        (p.volPaired && p.volPaired.toLowerCase().includes(query)),
    );
  ...
  eligibleTrainees.forEach((p) => {
    const currentBus = commAttData.busAttendance[juncture]
      ? commAttData.busAttendance[juncture][p.name]
      : "";
...
function generateBusCard(p, isBoarded, assignedBus) {
  ...
  if (p.volPaired) {
    ...
    if (vols.length > 0) {
      volHtml = vols
        .map(
          (v) =>
            `<span class="text-[9px] text-teal-700 dark:text-teal-400 font-bold bg-teal-50 dark:bg-teal-900/30 px-1 py-0.5 rounded border-2 border-teal-200 dark:border-teal-800/50 break-words w-fit"><i class="fa-solid fa-handshake-angle mr-1"></i>${v}</span>`,
        )
        .join("");
    }
  } else {
  ...
  const checkBtnClass = isBoarded
    ? "bg-green-500 border-green-600 text-white shadow-inner"
    : "bg-gray-50 dark:bg-black border-gray-300 dark:border-zinc-600 text-transparent";

  let busBadge = isBoarded
    ? `<span class="text-[10px] font-black uppercase text-yellow-800 dark:text-yellow-400 bg-yellow-100 dark:bg-yellow-900/50 px-1.5 py-0.5 rounded border-2 border-yellow-300 dark:border-yellow-700">${assignedBus}</span>`
    : "";
...
function updateBusCardDOM(name) {
  ...
  let targetListId =
    isBoarded && (filterBus === "ALL" || filterBus === currentBus)
      ? "busBoardedList"
      : "busNotBoardedList";
...
function toggleBusStatus(name, forceBoarded, e) {
  ...
  if (forceBoarded) {
    ...
    if (filterBus === "ALL") {
      showFlashMessage(
        "busGlobalStatus",
        "Please select a specific bus first.",
        "error",
      );
  ...
  lastCommAttLocalChange = Date.now();
  if (!commAttData.busAttendance[juncture])
    commAttData.busAttendance[juncture] = {};
  ...
  if (busState.searchQuery) {
    ...
    setTimeout(() => {
      const cardNode = document.getElementById(
        `bus-att-card-${name.replace(/[^a-zA-Z0-9]/g, "")}`,
      );
      if (cardNode) {
        scrollCardIntoViewLocally(cardNode);
        setTimeout(() => {
...
function promptNewBusJuncture() {
  ...
  apiCall("addCommJuncture", {
    sheetUrl: currentCommAttSheetUrl,
    junctureName: "__BUS__" + name.trim(),
  }).then((res) => {
    overlay.classList.add("hidden");
    if (res.success) {
      commAttData = res;
...
function promptDeleteBusJuncture() {
  ...
  if (
    !confirm(
      `Are you sure you want to delete the bus tracking for "${juncture}"?`,
    )
  )
    return;
...
function generateBusColumnText(columnType) {
  ...
  if (columnType === "boarded")
    listTitle = `BOARDED (${filterBus === "ALL" ? "All Buses" : filterBus})`;
