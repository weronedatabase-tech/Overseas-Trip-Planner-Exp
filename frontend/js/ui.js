...
function getProjectColor(groupName) {
  ...
  if (
    ...
    appSettings.projectColors[groupName]
  )
    return appSettings.projectColors[groupName];
  return "bg-gray-100 border-gray-300 text-gray-900 dark:bg-gray-800 dark:border-gray-600 dark:text-gray-100";
}

function getProjectAbbreviation(name) {
  const match = name.match(/\((.*?)\)/);
  if (match && match[1]) return match[1].substring(0, 3).toUpperCase();
...
function renderHeaderLegend() {
  ...
  if (
    !appSettings ||
    !appSettings.activeProjects ||
    appSettings.activeProjects.length === 0
  ) {
  ...
  appSettings.activeProjects.forEach((proj) => {
    if (!proj) return;
    const colorCls = getProjectColor(proj);
    const shortName = getProjectAbbreviation(proj);
    html += `<span class="px-1.5 py-0.5 rounded text-[11px] md:text-xs font-bold border shadow-md cursor-help ${colorCls}" title="${proj}">${shortName}</span>`;
  });
  ...
  while (changed) {
    ...
    for (let p of allParticipants) {
      ...
      if (p.role === "CAREGIVER" && p.relatedTrainee) {
        pRelated = String(p.relatedTrainee)
          .split("|")
      ...
      for (let f of family) {
        ...
        if (f.role === "CAREGIVER" && f.relatedTrainee) {
          fRelated = String(f.relatedTrainee)
            .split("|")
            .map((n) => n.replace(/\s+/g, "").toLowerCase())
            .filter(Boolean);
        }
          ...
          if (
            pRelated.some(
              (d) =>
                d.includes(fName) ||
                fName.includes(d) ||
                (fShortName && d.includes(fShortName)),
            )
          ) {
            match = true;
          }
          ...
          if (
            fRelated.some(
              (d) =>
                d.includes(pName) ||
                pName.includes(d) ||
                (pShortName && d.includes(pShortName)),
            )
          ) {
            match = true;
          }
...
function applyGlobalSorting(participants) {
  ...
  const rules = appSettings.sortingRules || [
    "project",
    "family",
    "role",
    "name",
  ...
  return participants.sort((a, b) => {
    for (let rule of rules) {
      ...
      if (rule === "name") {
        const cmp = (a.displayName || a.name || "").localeCompare(
...
function processDisplayNames(participants) {
  ...
  participants.forEach((p) => {
    if (displayCounts[p.displayName] > 1) {
      ...
      let extraChar = "";
      if (words.length > 1) {
        const diffWord = words.find(
          (w) => w.toLowerCase() !== sName.toLowerCase(),
        );
...
window.renderPhoneLink = function (phone, extraClasses = "") {
  ...
  return `<span class="inline-flex items-center gap-2 ${extraClasses}">
        ...
        <span class="inline-flex items-center gap-1.5 shrink-0">
            <a href="https://wa.me/${cleaned}" target="_blank" class="text-green-500 hover:text-green-600 dark:text-green-400 dark:hover:text-green-300 transition-colors bg-green-50 dark:bg-green-900/30 p-1.5 rounded-md border-2 border-green-200 dark:border-green-800/50 shadow-md" title="Chat on WhatsApp" onclick="event.stopPropagation()">
            ...
            <a href="tel:${dialNum}" title="Call" class="text-blue-500 hover:text-blue-600 dark:text-blue-400 dark:hover:text-blue-300 transition-colors bg-blue-50 dark:bg-blue-900/30 p-1.5 rounded-md border-2 border-blue-200 dark:border-blue-800/50 shadow-md" onclick="event.stopPropagation()">
...
window.setupTokenInput = function (inputId, getSuggestionsCallback) {
  ...
  wrapper.className =
    "flex flex-wrap items-center gap-1.5 w-full p-2 border-2 border-gray-300 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-900 focus-within:ring-1 focus-within:ring-primary cursor-text min-h-[42px]";
  ...
  inputField.className =
    "flex-1 bg-transparent border-none focus:outline-none focus:ring-0 text-xs font-semibold text-gray-900 dark:text-white min-w-[60px] p-0";
  inputField.placeholder = "Search trainee...";

  const dropdown = document.createElement("div");
  dropdown.className =
    "absolute z-50 w-full bg-white dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-700 rounded-lg shadow-xl mt-1 hidden-force max-h-48 overflow-y-auto";
  ...
  let tokens = (originalInput.value || "")
    .split("|")
    .map((s) => s.trim())
    .filter(Boolean);
  ...
  function renderTokens() {
    ...
    const currentTokens = window._tokenInputs[inputId]
      ? window._tokenInputs[inputId].tokens
      : tokens;
    currentTokens.forEach((t, i) => {
      const chip = document.createElement("span");
      chip.className =
        "inline-flex items-center px-2 py-1 rounded-md text-xs font-black bg-blue-100 text-blue-800 dark:bg-blue-900/40 dark:text-blue-300 uppercase tracking-widest";
      chip.innerHTML = `
                ${t}
  ...
  function renderDropdown(suggestions) {
    if (!suggestions || suggestions.length === 0) {
      dropdown.innerHTML =
        '<div class="p-2 text-xs text-gray-500 text-center italic pointer-events-none">No matches found</div>';
    } else {
      dropdown.innerHTML = "";
      suggestions.forEach((s) => {
        const item = document.createElement("div");
        item.className =
          "px-3 py-2 text-sm font-bold text-gray-800 dark:text-gray-200 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 transition border-b-2 border-gray-100 dark:border-gray-700 last:border-0";
  ...
  inputField.addEventListener("focus", () => {
    const suggestions = getSuggestionsCallback(
...
const OTHER_NATIONALITIES = ALL_NATIONALITIES.filter(
  (n) => !TOP_NATIONALITIES.includes(n),
...
window.setupNationalityDropdown = function (inputId) {
  ...
  displayBtn.className =
    "w-full p-2.5 border-2 border-gray-300 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white focus:outline-none focus:ring-1 focus:ring-primary flex justify-between items-center text-left min-h-[42px] transition";

  const displaySpan = document.createElement("span");
  displaySpan.className =
    "truncate text-sm " + (input.value ? "font-semibold" : "text-gray-400");
  ...
  manualInput.className =
    "w-full p-2.5 pr-10 border-2 border-gray-300 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white focus:outline-none focus:ring-1 focus:ring-primary text-sm font-semibold";

  const manualClearBtn = document.createElement("button");
  manualClearBtn.type = "button";
  manualClearBtn.className =
    "absolute right-2 p-1 text-gray-400 hover:text-red-500 focus:outline-none";
  ...
  const dropdown = document.createElement("div");
  dropdown.className =
    "absolute z-[100] w-full mt-1 bg-white dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-700 rounded-lg shadow-xl hidden-force flex flex-col max-h-64 overflow-hidden";

  const searchHeader = document.createElement("div");
  searchHeader.className =
    "p-2 border-b-2 border-gray-100 dark:border-gray-700 flex items-center gap-2 bg-gray-50 dark:bg-gray-900/50";

  const searchInput = document.createElement("input");
  searchInput.type = "text";
  searchInput.placeholder = "Search...";
  searchInput.className =
    "flex-1 p-1.5 text-sm border-2 border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:border-primary";

  const clearBtn = document.createElement("button");
  clearBtn.type = "button";
  clearBtn.title = "Clear Field";
  clearBtn.className =
    "p-1.5 text-gray-400 hover:text-red-500 transition focus:outline-none";
  clearBtn.innerHTML =
    '<svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>';
  ...
  const setManualMode = (isActive) => {
    if (isActive) {
      ...
      manualInput.value =
        input.value && input.value !== "Others" ? input.value : "";
  ...
  if (
    input.value &&
    !window.NATIONALITIES_LIST.includes(input.value) &&
    input.value !== "Others"
  ) {
    setManualMode(true);
  }
  ...
  const renderList = (query) => {
    ...
    const filtered = window.NATIONALITIES_LIST.filter(
      (n) => n !== "Others" && n.toLowerCase().includes(q),
    );
    ...
    const mappedFiltered = filtered
      .map((n) => {
        ...
        let styling = isTop
          ? "font-bold text-gray-900 dark:text-white bg-gray-50 dark:bg-gray-900/30"
    ...
    if (q === "") {
      ...
      if (filtered.length === 0) {
        html +=
          '<div class="p-3 text-xs text-center text-gray-500">No results found in list. Use "Others" below to specify manually.</div>';
      } else {
        html += mappedFiltered;
      }
      html += othersHtml;
...
function initDatePicker() {
  if (!document.getElementById("datePickerSheet")) {
    ...
    div.className =
      "fixed inset-0 bg-black/60 z-[120] hidden-force flex flex-col justify-end";
    div.innerHTML = `<div class="bg-white dark:bg-gray-900 rounded-t-3xl w-full max-w-md mx-auto overflow-hidden shadow-2xl animate-slide-up border-t-2 border-gray-200 dark:border-gray-800"><div class="flex justify-between items-center p-5 border-b-2 border-gray-200 dark:border-gray-800"><span class="font-bold text-lg text-gray-800 dark:text-gray-100">Select Date</span><button type="button" onclick="closePicker()" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 text-2xl font-bold px-2 focus:outline-none">&times;</button></div><div class="relative flex h-[200px] text-lg font-bold bg-white dark:bg-gray-950"><div class="picker-highlight"></div><div class="flex-1 picker-col" id="colDay"></div><div class="flex-1 picker-col" id="colMonth"></div><div class="flex-1 picker-col" id="colYear"></div></div><div class="p-5 border-t-2 border-gray-200 dark:border-gray-800"><button type="button" onclick="confirmPicker()" class="w-full bg-primary text-white py-3.5 rounded-xl font-bold text-lg shadow-md focus:outline-none hover:bg-green-600 transition">Done</button></div></div>`;
...
window.openDatePicker = function openDatePicker(targetId, type) {
  ...
  setTimeout(() => {
    if (curVal) {
      ...
      if (parts.length === 3) {
        const dIdx = Array.from(
          colD.querySelectorAll(".picker-item"),
        ).findIndex((el) => el.dataset.val === parts[0]);
        const mIdx = Array.from(
          colM.querySelectorAll(".picker-item"),
        ).findIndex((el) => el.dataset.val === parts[1]);
        const yIdx = Array.from(
          colY.querySelectorAll(".picker-item"),
        ).findIndex((el) => el.dataset.val === parts[2]);
    ...
    highlightCenterItem("colMonth");
    highlightCenterItem("colYear");
    highlightCenterItem("colDay");
  }, 10);
};
...
window.scrollToIndex = function scrollToIndex(colId, index) {
  document
    ...
    .scrollTo({ top: index * 40, behavior: "smooth" });
};

function highlightCenterItem(colId) {
  const col = document.getElementById(colId);
  const visibleItems = Array.from(col.querySelectorAll(".picker-item")).filter(
    (i) => !i.classList.contains("hidden-force"),
  );
...
function updateValidDays() {
  const mItem = document.querySelector(
    '#colMonth .picker-item[data-selected="true"]',
  );
  const yItem = document.querySelector(
    '#colYear .picker-item[data-selected="true"]',
  );
  if (!mItem || !yItem) return;

  const maxDays = new Date(
    parseInt(yItem.dataset.val),
    monthsArr.indexOf(mItem.dataset.val) + 1,
    0,
  ).getDate();
  const colD = document.getElementById("colDay");
  ...
  colD.querySelectorAll(".picker-item").forEach((item) => {
    ...
    if (shouldHide && !item.classList.contains("hidden-force"))
...
window.confirmPicker = function confirmPicker() {
  const d = document.querySelector(
    '#colDay .picker-item[data-selected="true"]',
  );
  ...
  if (d && m && y && currentPickerTarget) {
    document.getElementById(currentPickerTarget).value =
      `${d.dataset.val} ${m.dataset.val} ${y.dataset.val}`;
  }
  closePicker();
};
