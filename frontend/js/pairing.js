...
function loadManualPairingData() {
  ...
  apiCall("fetchManualPairingData", {
    ...
    if (res.success) {
      ...
      if (
        isFilteredManualPairingMode &&
        filteredManualPairingSourceView === "tracker.html"
      ) {
        window.navigateTo("tracker.html", {
          url: currentManualPairingSheetUrl,
        });
      } else {
...
function triggerManualPairingPulse(sourceName, targetName, isPaired) {
  setTimeout(() => {
    requestAnimationFrame(() => {
      const querySource = sourceName
        .replace(/\\/g, "\\\\")
        .replace(/"/g, '\\"');
      const queryTarget = targetName
        .replace(/\\/g, "\\\\")
        .replace(/"/g, '\\"');

      const sourceCard = document.querySelector(
        `.dnd-dropzone[data-name="${querySource}"]`,
      );
      const targetCard = document.querySelector(
        `.dnd-dropzone[data-name="${queryTarget}"]`,
      );
...
function generatePairingPillHtml(
  pillName,
  traineeName,
  volName,
  isTraineeGoneHome = false,
) {
  const goneHomeBadge = isTraineeGoneHome
    ? `<i class="fa-solid fa-house-user text-blue-500 ml-1" title="Gone Home"></i>`
    : "";

  const jsSafeTrainee = traineeName
    .replace(/\\/g, "\\\\")
    .replace(/'/g, "\\'")
    .replace(/"/g, "&quot;");
  const jsSafeVol = volName
    .replace(/\\/g, "\\\\")
    .replace(/'/g, "\\'")
    .replace(/"/g, "&quot;");

  const removeBtn = isTraineeGoneHome
    ? ""
    : `<div class="remove-x flex items-center justify-center font-bold text-[10px] bg-transparent text-red-500 shadow-none border-none hover:bg-transparent hover:text-red-700 hover:scale-125 top-0 right-1" onclick="unpairTrainee('${jsSafeTrainee}', '${jsSafeVol}')">✕</div>`;
...
function generatePairingCardHtml(item, pairedNames) {
  ...
  pairedNames.forEach((pairedName) => {
    ...
    if (isVol) {
      const traineeObj = (manualPairingData.trainees || []).find(
        (t) => t.name === tName,
      );
    ...
    pairedPills += generatePairingPillHtml(
      pairedName,
      tName,
      vName,
      isTraineeGoneHome,
...
function insertPairingCardSorted(container, newCardNode, itemData, role) {
  const children = Array.from(container.children).filter((c) =>
    c.classList.contains("dnd-dropzone"),
  );
  let inserted = false;
  ...
  for (let child of children) {
    ...
    const childItem = (
      manualPairingData[role === "VOLUNTEER" ? "volunteers" : "trainees"] || []
    ).find((x) => x.name === childName);
...
function updateGlobalUnpairedCount() {
  ...
  (manualPairingData.trainees || []).forEach((t) => {
    ...
    if (
      att === "y" &&
      !t.isGoneHome &&
      (!t.volPaired || t.volPaired.trim() === "")
    ) {
      count++;
    }
  });
  if (typeof updateUnpairedNotification === "function")
...
function updatePairingCardDOM(name, role) {
  const arr =
    role === "VOLUNTEER"
      ? manualPairingData.volunteers
      : manualPairingData.trainees;
  ...
  if (role === "VOLUNTEER" && showOnlyUnpairedVols && pairedNames.length > 0)
    shouldBeInDOM = false;
  if (
    role === "TRAINEE" &&
    isFilteredManualPairingMode &&
    ...
    shouldBeInDOM = false;
  if (role === "TRAINEE" && item.isGoneHome && isFilteredManualPairingMode)
    shouldBeInDOM = false;

  const volSearchQuery =
    document.getElementById("pairingVolSearch")?.value.toLowerCase().trim() ||
    "";
  const traSearchQuery =
    document
      .getElementById("pairingTraineeSearch")
      ?.value.toLowerCase()
      .trim() || "";

  if (role === "VOLUNTEER" && volSearchQuery) {
    if (
      !item.name.toLowerCase().includes(volSearchQuery) &&
      !(item.project && item.project.toLowerCase().includes(volSearchQuery))
    ) {
      shouldBeInDOM = false;
    }
  }
  if (role === "TRAINEE" && traSearchQuery) {
    if (
      !item.name.toLowerCase().includes(traSearchQuery) &&
      !(item.project && item.project.toLowerCase().includes(traSearchQuery)) &&
      !(item.group && String(item.group).toLowerCase().includes(traSearchQuery))
    ) {
      shouldBeInDOM = false;
    }
  }
  ...
  if (existingCard) {
    if (!shouldBeInDOM) {
      ...
      setTimeout(() => {
        existingCard.remove();
        const containerId =
          role === "VOLUNTEER" ? "dnd-source-pool" : "dnd-target-list";
        const container = document.getElementById(containerId);
        if (
          container &&
          container.querySelectorAll(".dnd-dropzone").length === 0
        ) {
    ...
    if (shouldBeInDOM) {
      ...
      if (container) {
        ...
        uiBindLongPress(newCardNode, () => {
          const p = (
            manualPairingData[
              role === "VOLUNTEER" ? "volunteers" : "trainees"
            ] || []
          ).find((x) => x.name === item.name);
          if (p) showPersonInfo(p);
        });
...
function renderManualPairings() {
  ...
  const sortFn = (a, b) => {
    const projA = a.project
      ? a.project.toString().toLowerCase().trim()
      : "zzzz";
    const projB = b.project
      ? b.project.toString().toLowerCase().trim()
      : "zzzz";
  ...
  if (isFilteredManualPairingMode) {
    trainees = trainees.filter(
      (t) => !t.isGoneHome && (!t.volPaired || t.volPaired.trim() === ""),
    );
  }
  ...
  if (sourcePool) {
    sourcePool.innerHTML =
      sourceHtml ||
      '<p class="text-[10px] text-gray-500 font-bold p-2 text-center mt-2">No active volunteers matching search.</p>';
  }

  let targetHtml = "";
  trainees.forEach((item) => {
    const myVols = item.volPaired
      ? item.volPaired
          .split(/[,|\n]+/)
          .map((v) => v.trim())
          .filter((v) => v)
  ...
  if (targetList) {
    if (isFilteredManualPairingMode && targetHtml === "") {
      targetList.innerHTML =
        '<p class="text-[10px] text-green-500 font-bold p-2 text-center mt-2">All trainees are paired!</p>';
    } else {
      targetList.innerHTML =
        targetHtml ||
        '<p class="text-[10px] text-gray-500 font-bold p-2 text-center mt-2">No active trainees matching search.</p>';
    }
  }
...
function handleManualPairingDrop(sourceName, sourceRole, targetName) {
  ...
  if (blockingTraineeName) {
    showFlashMessage(
      "pairingGlobalStatus",
      `Cannot pair! ${volName} is already paired with ${blockingTraineeName} in a different group.`,
      "error",
    );
    return;
  }

  const currentVols = trainee.volPaired
    ? trainee.volPaired
        .split(/[,|\n]+/)
        .map((v) => v.trim())
        .filter((v) => v)
  ...
  if (!exists) {
    ...
    const updateIndex = pendingPairingUpdates.findIndex(
      (u) => u.traineeName === traineeName,
    );
    if (updateIndex > -1) {
      pendingPairingUpdates[updateIndex].volPaired = trainee.volPaired;
    } else {
      pendingPairingUpdates.push({
        traineeName: traineeName,
        volPaired: trainee.volPaired,
      });
    }
...
function unpairTrainee(traineeName, volName) {
  ...
  let currentVols = trainee.volPaired
    ? trainee.volPaired
        .split(/[,|\n]+/)
        .map((v) => v.trim())
        .filter((v) => v)
...
async function executeManualPairingSync() {
  ...
  try {
    const res = await apiCall("syncManualPairingUpdates", {
      sheetUrl: currentManualPairingSheetUrl,
    ...
    updatesToSync.forEach((u) => {
      const idx = pendingPairingUpdates.findIndex(
        (p) => p.traineeName === u.traineeName,
      );
...
function startManualPairingPolling() {
  ...
  manualPairingPollInterval = setInterval(async () => {
    if (
      isManualPairingSyncing ||
      (window.dndState && (window.dndState.el || window.dndState.isDragging))
    )
      return;
    if (pendingPairingUpdates.length > 0) return;
    ...
    try {
      ...
      if (
        res.success &&
        !isManualPairingSyncing &&
        pendingPairingUpdates.length === 0
      ) {
...
function filterQuickPairList() {
  ...
  if (matches.length === 0) {
    listEl.innerHTML =
      '<li class="text-xs text-gray-500 p-2 text-center">No matches found</li>';
    return;
  }
  ...
  matches.forEach((name) => {
    let isPaired = false;
    const traineeName =
      quickPairContext.sourceRole === "TRAINEE"
        ? quickPairContext.sourceName
        : name;
    const volName =
      quickPairContext.sourceRole === "VOLUNTEER"
        ? quickPairContext.sourceName
        : name;

    const trainee = manualPairingData.trainees.find(
      (t) => t.name === traineeName,
    );
    if (trainee && trainee.volPaired) {
      const vols = trainee.volPaired
        .split(/[,|\n]+/)
        .map((v) => v.trim())
        .filter((v) => v);
    ...
    if (!isPaired) {
      li.onclick = () => {
        handleManualPairingDrop(
          quickPairContext.sourceName,
          quickPairContext.sourceRole,
          name,
        );
        closeQuickPairModal();
      };
