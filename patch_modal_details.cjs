...
// Extract the existing grid section
const match = code.match(/<div class="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-3 text-sm text-gray-800 dark:text-gray-200">([\s\S]*?)<\/div>\s*<\/div>\s*<form id="gpm-edit"/);
...
`;
    ...
    code = code.replace(match[1], newGridHtml);
    fs.writeFileSync('frontend/js/main.js', code);
    console.log('Patched Participant details ordering');
} else {
    console.log('Failed to match participant details grid');
}
