...
const regex = /<div class="grid grid-cols-1 md:grid-cols-2 gap-3" id="myGroupMembersGrid">` \+\s*loadedGroupMembers\.map\(member => \{\s*return `([\s\S]*?)`\s*\}\)\.join\(""\) \+/g;
...
const replacement = `<div class="grid grid-cols-1 md:grid-cols-2 gap-3" id="myGroupMembersGrid">\` + 
        loadedGroupMembers.map(member => {
            ...
            if (member.role === 'TRAINEE') {
                medicalHtml = \\\`<div class="text-[11px] font-bold text-gray-500 dark:text-gray-400 flex items-start gap-1.5 mt-0.5"><i class="fa-solid fa-notes-medical w-4 text-center mt-0.5 text-red-400"></i> <span class="text-gray-700 dark:text-gray-300 leading-tight">\\\${member.medical || "None"}</span></div>\\\`;
            }
            ...
            if (member.role === 'TRAINEE' && globalLogistics && globalLogistics.pairings && globalLogistics.participants) {
                ...
                if (pairs.length > 0) {
                    ...
                    if (vols) {
                        pairedVolunteersStr = \\\`<div class="text-[11px] font-bold text-gray-500 dark:text-gray-400 flex items-start gap-1.5 mt-0.5"><i class="fa-solid fa-hands-holding-child w-4 text-center mt-0.5 text-blue-400"></i> <span class="text-gray-700 dark:text-gray-300 leading-tight">\\\${vols}</span></div>\\\`;
                    }
                }
            }
