...
const getFinanceDataStart = /function fetchFinance\(forceRebuild = false\) \{/g;
const repairLogic = `
...
code = code.replace(/function fetchFinance\(forceRebuild = false\) \{[\s\S]*?let ratesObj = \{ "SGD": 1 \};/, repairLogic + '\nlet ratesObj = { "SGD": 1 };');

fs.writeFileSync('backend/Code.js', code);
console.log('Patched fetchFinance with repair');
